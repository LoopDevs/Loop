...
export const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  (
    { variant = 'primary', size = 'md', disabled, loading, leftIcon, rightIcon, children, className = '', type = 'button', ...props },
    ref,
  ) => {
    const base = 'inline-flex items-center justify-center gap-2 rounded-lg font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed';
    const classes = `${base} ${VARIANTS[variant]} ${SIZES[size]} ${className}`.trim();

    return (
      <button ref={ref} type={type} disabled={disabled === true || loading === true} className={classes} {...props}>
        {loading === true && (
