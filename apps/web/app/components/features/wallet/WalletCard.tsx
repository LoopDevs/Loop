...
import { USER_LOCALE } from '~/utils/locale';
 ...
 */
export function fmtLoopBalance(balance: string, assetCode: string): string {
  if (!isLoopAssetCode(assetCode)) return `${balance} ${assetCode}`;
  const currency = currencyForLoopAsset(assetCode);
  const n = Number(balance);
  if (!Number.isFinite(n)) return '—';
  try {
    return new Intl.NumberFormat(USER_LOCALE, { style: 'currency', currency }).format(n);
  } catch {
      ...
      {!isActivated ? (
        ...
        <>
          {loopRows.map((row, i) => (
            ...
            >
              {fmtLoopBalance(row.balance, row.assetCode)}
            </p>
          ))}
          <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
