...
async function decodeProtobufClusters(buffer: ArrayBuffer): Promise<ClusterResponse> {
  // A4-115: protobuf-es v2 exports a Schema descriptor, not a class.
  // The legacy `ProtobufClusterResponse.fromBinary(...)` call would
  // throw `ProtobufClusterResponse is undefined` if the protobuf
  // path were ever reached — backend always fell back to JSON
  // because of a parallel symbol-shape bug, masking this until
  // A4-115. Now using `fromBinary(Schema, bytes)`.
  /* eslint-disable @typescript-eslint/no-explicit-any */
  const { fromBinary } = (await import('@bufbuild/protobuf' as any)) as any;
  const { ProtobufClusterResponseSchema } = (await import(
    '@loop/shared/src/proto/clustering_pb.js' as any
  )) as any;
  /* eslint-enable @typescript-eslint/no-explicit-any */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const msg = fromBinary(ProtobufClusterResponseSchema, new Uint8Array(buffer)) as any;
