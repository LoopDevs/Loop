...
#   - Deep linking (M-3) — Android App Links intent-filter
#     (ACTION_VIEW + BROWSABLE + DEFAULT, autoVerify, loopfinance.io /
#     www / beta hosts) patched into AndroidManifest.xml, and iOS
#     associated-domains: App.entitlements copied in + wired via
#     CODE_SIGN_ENTITLEMENTS on both build configurations in
#     project.pbxproj. Both are `cap add`-hostile the same way the
#     M-4 items are — a fresh regeneration has no intent-filter and no
#     entitlements file at all. Domain ownership is proven at runtime
#     by the backend's `.well-known/apple-app-site-association` +
#     `assetlinks.json` (apps/backend/src/well-known/), gated on
#     operator-filled APPLE_TEAM_ID / ANDROID_CERT_SHA256.
#
...
apply from: 'signing.gradle'|" "$ANDROID_BUILD_GRADLE"
  ...
  # ─── Android App Links intent-filter (M-3, deep linking) ────────────────
  # A second <intent-filter> on MainActivity, additive to the existing
  # MAIN/LAUNCHER one — ACTION_VIEW + BROWSABLE + DEFAULT with
  # autoVerify="true" so tapping an https://loopfinance.io/... link (or
  # www / beta) opens the app directly instead of the browser, once the
  # backend's assetlinks.json (apps/backend/src/well-known/) proves
  # domain ownership. Anchored on the MAIN/LAUNCHER intent-filter block
  # (Capacitor's android-template ships it verbatim on `cap add
  # android`) via regex rather than a hardcoded line number, so the
  # insert survives template churn; idempotent via the loopfinance.io
  # host check. Multi-line block insert uses python3 (same tool the iOS
  # pbxproj patch below requires) rather than sed — more robust across
  # GNU/BSD sed than a backslash-continued multi-line -i edit.
  if ! grep -q 'android:host="loopfinance.io"' "$ANDROID_MANIFEST"; then
    if ! command -v python3 >/dev/null 2>&1; then
      say "ERROR: python3 is required for the Android App Links intent-filter patch (M-3)."
      exit 1
    fi
    say "Adding App Links intent-filter (M-3) to AndroidManifest.xml"
...
if not m:
    sys.stderr.write(
        "ERROR: MAIN/LAUNCHER intent-filter not found in AndroidManifest.xml — "
        "Capacitor template drifted. Update the M-3 patch in "
...
block = (
    ...
    f'{indent}    <data android:scheme="https" android:host="loopfinance.io" />\n'
    f'{indent}    <data android:scheme="https" android:host="www.loopfinance.io" />\n'
    f'{indent}    <data android:scheme="https" android:host="beta.loopfinance.io" />\n'
...
with open(path, "w") as f:
    f.write(text)
print("[apply-native-overlays] Added App Links intent-filter to AndroidManifest.xml")
PY
    if ! grep -q 'android:host="loopfinance.io"' "$ANDROID_MANIFEST"; then
      say "ERROR: python3 patch completed but the App Links intent-filter is still"
...
if [ -f "$IOS_PLIST" ]; then
  ...
  # Associated-domains entitlements (M-3, deep linking). Declares the
  # applinks: hosts Universal Links may open the app for; Apple
  # verifies each against `apple-app-site-association` served by the
  # backend. `cap add ios` has no notion of this file at all (unlike
  # Info.plist / PrivacyInfo.xcprivacy, which the template ships and
  # this script only edits) — CODE_SIGN_ENTITLEMENTS wiring happens in
  # the pbxproj patch below.
  IOS_ENTITLEMENTS_SRC="$OVERLAY_DIR/ios/App/App/App.entitlements"
  IOS_ENTITLEMENTS_DEST="$ROOT_DIR/apps/mobile/ios/App/App/App.entitlements"
  if [ -f "$IOS_ENTITLEMENTS_SRC" ]; then
    cp_if_changed "$IOS_ENTITLEMENTS_SRC" "$IOS_ENTITLEMENTS_DEST"
    say "Copied iOS App.entitlements (M-3)"
  fi

  # ─── iOS: pbxproj wiring for release.xcconfig + PrivacyInfo.xcprivacy + App.entitlements ────
  # (M-4, extended by M-3) These steps were "operator-once in Xcode" —
  # see the comments above. All are store-critical (a Release build
  # silently keeping CAPACITOR_DEBUG=true, an App-Store binary
  # rejected for a missing Privacy Manifest, or Universal Links
  # silently not working because no entitlement declares the
  # associated domains) and none are regenerated by a bare `cap sync`,
  # so leaving them manual was a silent regression waiting to happen.
  # This patches the plist-format project.pbxproj directly: anchored
  # on content that must already exist (the debug.xcconfig
  # PBXFileReference, the Info.plist PBXFileReference, the
  # `/* Release */` XCBuildConfiguration blocks, the
  # PRODUCT_BUNDLE_IDENTIFIER build setting) rather than on hardcoded
  # object ids — Capacitor's ios-template ships the file verbatim on
  # `cap add ios` so ids are stable in practice, but anchoring on
  # names/comments survives an id bump too. Idempotent, and fails
  # loudly (non-zero exit) if an expected anchor is missing rather
  # than silently no-op'ing, and again if post-patch verification
  # doesn't find what it just wrote.
...
ENTITLEMENTS_FILEREF_ID = "F00DBEEF0000000000000004"
...
# ── 8. App.entitlements PBXFileReference (sibling of Info.plist, M-3) ──
if "/* App.entitlements */ = {isa = PBXFileReference" not in text:
    ...
    if not m:
        fail("Info.plist PBXFileReference not found — cannot place App.entitlements alongside it")
    assert_fresh_id(ENTITLEMENTS_FILEREF_ID)
    indent = m.group(1)
    new_line = (
        f'{indent}{ENTITLEMENTS_FILEREF_ID} /* App.entitlements */ = {{isa = PBXFileReference; '
        f'lastKnownFileType = text.plist.entitlements; path = App.entitlements; sourceTree = "<group>"; }};\n'
    )
    text = text[: m.end()] + new_line + text[m.end() :]
    changed.append("App.entitlements PBXFileReference")

# ── 9. App.entitlements group membership (sibling of Info.plist entry, M-3) ──
if f"{ENTITLEMENTS_FILEREF_ID} /* App.entitlements */," not in text:
    m = re.search(r'^([ \t]*)([0-9A-F]{24}) /\* Info\.plist \*/,\n', text, re.MULTILINE)
    if not m:
        fail("Info.plist is not a child of any PBXGroup — cannot place App.entitlements alongside it")
    indent = m.group(1)
    new_line = f'{indent}{ENTITLEMENTS_FILEREF_ID} /* App.entitlements */,\n'
    text = text[: m.end()] + new_line + text[m.end() :]
    changed.append("App.entitlements PBXGroup membership")

# ── 10. CODE_SIGN_ENTITLEMENTS on the App target's Debug + Release build configs (M-3) ──
# Anchored on PRODUCT_BUNDLE_IDENTIFIER, which (unlike
# baseConfigurationReference/xcconfig) only appears in the App
# TARGET's two XCBuildConfiguration blocks, not the project-level
# ones — entitlements are a per-target build setting, so this
# deliberately does NOT touch the project-level Debug/Release blocks
# the release.xcconfig step above wires (those have no
# PRODUCT_BUNDLE_IDENTIFIER and are not code-signed independently).
if "CODE_SIGN_ENTITLEMENTS" not in text:
    bundle_id_pattern = re.compile(r'([ \t]*)PRODUCT_BUNDLE_IDENTIFIER = io\.loopfinance\.app;\n')
    bundle_matches = list(bundle_id_pattern.finditer(text))
    if not bundle_matches:
        fail(
            "PRODUCT_BUNDLE_IDENTIFIER = io.loopfinance.app not found in any "
            "XCBuildConfiguration — cannot wire CODE_SIGN_ENTITLEMENTS"
        )
    for m in reversed(bundle_matches):
        indent = m.group(1)
        new_line = f'{indent}CODE_SIGN_ENTITLEMENTS = App/App.entitlements;\n'
        text = text[: m.end()] + new_line + text[m.end() :]
    changed.append("CODE_SIGN_ENTITLEMENTS (Debug + Release)")
...
if "/* App.entitlements */ = {isa = PBXFileReference" not in text:
    errors.append("App.entitlements PBXFileReference missing after patch (M-3)")

entitlements_wiring_count = text.count("CODE_SIGN_ENTITLEMENTS = App/App.entitlements;")
if entitlements_wiring_count < 2:
    errors.append(
        "CODE_SIGN_ENTITLEMENTS = App/App.entitlements; is wired on fewer than 2 "
        f"XCBuildConfiguration blocks after patch (found {entitlements_wiring_count}, M-3)"
    )
...
if text != original_text:
    ...
    print(
        ...
        "(release.xcconfig + PrivacyInfo.xcprivacy + App.entitlements wiring), skipping write"
    )
PY
  else
