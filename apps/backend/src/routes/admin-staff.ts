 ...
 *
 * Admin-tier only: the grant/revoke writes are the self-serve
 * alternative to editing the config allowlist and redeploying, so they
 * carry the ADR 028 step-up gate ON TOP of the admin tier — a captured
 * bearer alone must not be able to mint itself a colleague — plus the
 * full ADR 017 envelope (idempotency, reason, Discord audit). The list
 * is admin-tier too: who holds power is itself sensitive, and support
 * has no action to take on it.
 *
 * Mount-order semantics shared with `mountAdminRoutes`: this factory
 * MUST be called AFTER the namespace middleware stack (cache-control /
 * requireAuth / requireStaff('support') blanket / audit middleware) is
 * in place; that's the parent factory's responsibility.
 */
...
export function mountAdminStaffRoutes(app: Hono): void {
  ...
  // deliberate ops action, and 10/min still leaves headroom for a bulk
  // onboarding session.
  app.put(
