...
--> statement-breakpoint
-- 2026-06-15 cold audit v-wallet P0 follow-up: pin interest_mint rows
-- to GBPLOOP at the DB layer too (app-layer allowlist lives in
-- ONCHAIN_MINT_ELIGIBLE_ASSETS, credits/interest-mint.ts). Widen
-- alongside that allowlist once vault-share accounting exists.
ALTER TABLE pending_payouts
  ADD CONSTRAINT pending_payouts_interest_mint_asset_pinned
  CHECK (kind != 'interest_mint' OR asset_code = 'GBPLOOP');
--> statement-breakpoint
CREATE TABLE "interest_mint_snapshots" (
  ...
  -- Pinned to GBPLOOP only (2026-06-15 cold audit v-wallet P0):
  -- USDLOOP/EURLOOP are DeFindex vault shares whose yield is price
  -- growth, not issuer-minted supply — see the allowlist doc comment
  -- on ONCHAIN_MINT_ELIGIBLE_ASSETS in credits/interest-mint.ts.
  -- Widen this CHECK only alongside that allowlist, once vault-share
  -- accounting exists.
  CONSTRAINT "interest_mint_snapshots_asset_code_known"
    CHECK ("asset_code" = 'GBPLOOP'),
