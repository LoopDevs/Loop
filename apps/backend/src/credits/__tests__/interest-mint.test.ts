...
describe('runInterestMintTick', () => {
  ...
  it('2026-06-15 cold audit v-wallet P0: never mints USDLOOP/EURLOOP even with a fully valid issuer signer configured', async () => {
    configMocks.configuredLoopPayableAssets.mockReturnValue([
      { code: 'USDLOOP', issuer: ISSUER },
      { code: 'EURLOOP', issuer: ISSUER },
    ]);
    configMocks.resolveIssuerSigners.mockReturnValue(
      new Map([
        ['USDLOOP', { secret: 'SSECRET', account: ISSUER }],
        ['EURLOOP', { secret: 'SSECRET', account: ISSUER }],
      ]),
    );
    state.eligibleUsers = [{ id: 'u-1', walletAddress: WALLET }];
    horizonState.balances.set(WALLET, 5_000_000_000n);
