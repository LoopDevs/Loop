...
async function seedFavorite(userId: string, merchantId: string, createdAt?: Date): Promise<void> {
  await db
    .collection('user_favorite_merchants')
    .insertOne({ userId, merchantId, createdAt: createdAt ?? new Date() });
}

describe('user favourites — document store', () => {
  beforeEach(() => {
    __resetDbForTests();
  });
  ...
  it('POST /favorites adds a row and is idempotent on re-add', async () => {
    ...
    const rows = await db.collection('user_favorite_merchants').findMany({ userId: me.userId });
  ...
  it('GET /favorites returns rows newest-first joined to the catalog', async () => {
    ...
    for (const [i, id] of (['amazon', 'starbucks', 'target'] as const).entries()) {
      await seedFavorite(me.userId, id, new Date(base + i * 1000));
    }
  ...
  it('GET /favorites surfaces merchant: null for catalog-evicted ids without crashing', async () => {
    ...
    await seedFavorite(me.userId, 'evicted-merchant');
  ...
  it('DELETE /favorites/:id removes a row and is idempotent on re-delete', async () => {
    ...
    await seedFavorite(me.userId, 'starbucks');
  ...
  it('caller A cannot read or remove caller B favourites (scope)', async () => {
    ...
    await seedFavorite(b.userId, 'amazon');
    ...
    const bRows = await db.collection('user_favorite_merchants').findMany({ userId: b.userId });
    expect(bRows).toHaveLength(1);
  });

  it('refuses an add past the 50-favourite cap with FAVORITES_LIMIT_EXCEEDED', async () => {
    const me = await seedUser('fav-cap@test.local');
    // Seed 50 rows directly.
    for (let i = 0; i < 50; i++) {
      await seedFavorite(me.userId, `seeded-${i}`);
    }
    const res = await app.request('http://localhost/api/users/me/favorites', {
      method: 'POST',
      headers: {
  ...
  it('the cap boundary is exact: the 50th add succeeds, the 51st is refused', async () => {
    const me = await seedUser('fav-boundary@test.local');
    ...
    const fiftieth = await app.request(url, {
      method: 'POST',
      headers,
      body: JSON.stringify({ merchantId: 'amazon' }),
    });
    ...
    const fiftyFirst = await app.request(url, {
      method: 'POST',
      headers,
      body: JSON.stringify({ merchantId: 'starbucks' }),
    });
  ...
  // The (userId, merchantId) unique spec is the backstop for the
  // same-merchant add race: two truly concurrent adds both pass the
  // pre-insert existence check, one insert wins, the loser's unique
  // violation is replayed as `added: false` — never a 500, never a
  // duplicate row.
  it('concurrent same-merchant adds land exactly one row (unique-spec backstop)', async () => {
    const me = await seedUser('fav-race@test.local');
    const url = 'http://localhost/api/users/me/favorites';
    const headers = {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${me.bearer}`,
    ...
    const [resA, resB] = await Promise.all([
      app.request(url, { method: 'POST', headers, body }),
      app.request(url, { method: 'POST', headers, body }),
    ]);
    expect(resA.status).toBe(200);
    expect(resB.status).toBe(200);
    const added = [
      ((await resA.json()) as { added: boolean }).added,
      ((await resB.json()) as { added: boolean }).added,
    ];
    // Exactly one of the two reported the actual insert.
