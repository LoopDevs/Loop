 ...
 *     (NOT registered — composed into `TreasurySnapshot` only)
...
export function registerAdminTreasuryAssetsOpenApi(
  ...
  const TreasuryOrderFlow = z.object({
    ...
    userCashbackMinor: z.string().openapi({
      description:
        'Cashback CTX applied as a checkout discount for this bucket. Minor units, bigint-string.',
    }),
    expectedCommissionMinor: z.string().openapi({
      description:
        'Commission Loop expects CTX to accrue for this bucket. Minor units, bigint-string.',
    }),
  });
  ...
  const TreasurySnapshot = registry.register(
    ...
    z.object({
      ...
      orderFlows: z.record(z.string(), TreasuryOrderFlow).openapi({
        description:
          'ADR 052 — aggregated economics of fulfilled orders, keyed by charge currency (face value / user cashback discount / expected commission).',
      }),
      ctxApi: z
        .object({
        ...
        .openapi({
          description:
            'ADR 051 — CTX upstream credential presence + circuit state (closed / half_open / open).',
        }),
    }),
  );
