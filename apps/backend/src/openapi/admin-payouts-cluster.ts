  ...
  registry.registerPath({
    ...
    request: {
      query: z.object({
        ...
        kind: z.enum(['order_cashback', 'emission', 'burn', 'interest_mint']).optional().openapi({
          description:
            'ADR-024 §2 / ADR 036 / ADR 031 discriminator filter. `order_cashback` = order-fulfilment payout; `emission` = admin on-chain backfill (no mirror debit); `burn` = redemption issuer-return; `interest_mint` = nightly issuer-signed interest mint. Omitted → all.',
        }),
      }),
    },
    responses: {
