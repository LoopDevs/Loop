import { describe, it, expect, vi, beforeEach } from 'vitest';
import type { Context } from 'hono';

const state = vi.hoisted(() => ({
  rows: [] as Array<Record<string, unknown>>,
  throwErr: null as Error | null,
  limitArg: 0,
}));

const limitMock = vi.fn(async (n: number) => {
  state.limitArg = n;
  if (state.throwErr !== null) throw state.throwErr;
  return state.rows;
});
const orderByMock = vi.fn(() => ({ limit: limitMock }));
const whereMock = vi.fn(() => ({ orderBy: orderByMock }));
const fromMock = vi.fn(() => ({ where: whereMock }));
const selectMock = vi.fn(() => ({ from: fromMock }));

vi.mock('../../db/client.js', () => ({
  db: { select: () => selectMock() },
}));

vi.mock('../../db/schema.js', () => ({
  orders: { createdAt: 'orders.created_at' },
}));

vi.mock('drizzle-orm', async () => {
  const actual = (await vi.importActual('drizzle-orm')) as Record<string, unknown>;
  return {
    ...actual,
    asc: (col: unknown) => ({ __asc: true, col }),
    sql: Object.assign(
      (strings: TemplateStringsArray, ...values: unknown[]) => ({ strings, values }),
      {},
    ),
  };
});

vi.mock('../../logger.js', () => ({
  logger: {
    child: () => ({ info: vi.fn(), warn: vi.fn(), error: vi.fn(), debug: vi.fn() }),
  },
}));

import { adminOrdersCsvHandler } from '../orders-csv.js';

function makeCtx(query: Record<string, string> = {}): Context {
  return {
    req: {
      query: (k: string) => query[k],
      param: (_k: string) => undefined,
    },
    json: (body: unknown, status?: number) =>
      new Response(JSON.stringify(body), {
        status: status ?? 200,
        headers: { 'content-type': 'application/json' },
      }),
  } as unknown as Context;
}

const baseRow = {
  id: 'o-1',
  userId: 'u-1',
  merchantId: 'm-1',
  state: 'fulfilled',
  currency: 'USD',
  faceValueMinor: 5000n,
  chargeCurrency: 'GBP',
  chargeMinor: 4000n,
  paymentMethod: 'loop_asset',
  wholesalePct: '80.00',
  userCashbackPct: '15.00',
  loopMarginPct: '5.00',
  wholesaleMinor: 3200n,
  userCashbackMinor: 600n,
  loopMarginMinor: 200n,
  ctxOrderId: 'ctx-abc',
  ctxOperatorId: 'op-1',
  failureReason: null,
  createdAt: new Date('2026-04-21T12:00:00Z'),
  paidAt: new Date('2026-04-21T12:05:00Z'),
  procuredAt: new Date('2026-04-21T12:06:00Z'),
  fulfilledAt: new Date('2026-04-21T12:07:00Z'),
  failedAt: null,
};

beforeEach(() => {
  state.rows = [];
  state.throwErr = null;
  state.limitArg = 0;
  limitMock.mockClear();
  orderByMock.mockClear();
  whereMock.mockClear();
  fromMock.mockClear();
  selectMock.mockClear();
});

describe('adminOrdersCsvHandler', () => {
  it('returns just the header when the window is empty', async () => {
    const res = await adminOrdersCsvHandler(makeCtx());
    expect(res.status).toBe(200);
    expect(res.headers.get('content-type')).toBe('text/csv; charset=utf-8');
    expect(res.headers.get('cache-control')).toBe('private, no-store');
    expect(res.headers.get('content-disposition')).toMatch(
      /^attachment; filename="orders-\d{4}-\d{2}-\d{2}\.csv"$/,
    );
    const body = await res.text();
    expect(body.startsWith('id,user_id,merchant_id')).toBe(true);
    expect(body.split('\r\n').filter((l) => l.length > 0)).toHaveLength(1);
  });

  it('never emits gift-card secrets — header has no redeem columns', async () => {
    const res = await adminOrdersCsvHandler(makeCtx());
    const body = await res.text();
    expect(body).not.toMatch(/redeem_code|redeem_pin|redeem_url/);
  });

  it('emits one row per order with bigint + ISO coercion', async () => {
    state.rows = [baseRow];
    const res = await adminOrdersCsvHandler(makeCtx());
    const body = await res.text();
    const lines = body.split('\r\n').filter((l) => l.length > 0);
    expect(lines).toHaveLength(2);
    const dataRow = lines[1]!;
    // Spot-check each bigint minor unit coerced, every timestamp ISO.
    expect(dataRow).toContain('5000'); // faceValueMinor
    expect(dataRow).toContain('4000'); // chargeMinor
    expect(dataRow).toContain('3200'); // wholesaleMinor
    expect(dataRow).toContain('600'); // userCashbackMinor
    expect(dataRow).toContain('200'); // loopMarginMinor
    expect(dataRow).toContain('2026-04-21T12:00:00.000Z');
    expect(dataRow).toContain('2026-04-21T12:07:00.000Z');
    // Null timestamps serialise as empty — no literal "null" text.
    expect(dataRow.endsWith(',')).toBe(true);
  });

  it('RFC 4180 — escapes commas / quotes in failure_reason', async () => {
    state.rows = [
      {
        ...baseRow,
        state: 'failed',
        failedAt: new Date('2026-04-21T14:00:00Z'),
        failureReason: 'boom: "quoted", comma, and CRLF\r\nhere',
      },
    ];
    const res = await adminOrdersCsvHandler(makeCtx());
    const body = await res.text();
    expect(body).toContain('"boom: ""quoted"", comma, and CRLF\r\nhere"');
  });

  it('truncates with __TRUNCATED__ past 10 000 rows', async () => {
    state.rows = Array.from({ length: 10_001 }, (_, i) => ({
      ...baseRow,
      id: `o-${i}`,
    }));
    const res = await adminOrdersCsvHandler(makeCtx());
    const body = await res.text();
    const lines = body.split('\r\n').filter((l) => l.length > 0);
    expect(lines).toHaveLength(10_002);
    expect(lines[lines.length - 1]).toBe('__TRUNCATED__');
  });

  it('echoes since in the filename', async () => {
    const res = await adminOrdersCsvHandler(makeCtx({ since: '2026-03-15T00:00:00Z' }));
    expect(res.headers.get('content-disposition')).toContain('orders-2026-03-15.csv');
  });

  it('400 on malformed since', async () => {
    const res = await adminOrdersCsvHandler(makeCtx({ since: 'not-a-date' }));
    expect(res.status).toBe(400);
  });

  it('400 when since is more than 366 days ago', async () => {
    const tooOld = new Date(Date.now() - 400 * 24 * 60 * 60 * 1000).toISOString();
    const res = await adminOrdersCsvHandler(makeCtx({ since: tooOld }));
    expect(res.status).toBe(400);
  });

  it('500 when the repo throws', async () => {
    state.throwErr = new Error('db exploded');
    const res = await adminOrdersCsvHandler(makeCtx());
    expect(res.status).toBe(500);
  });

  it('asks the repo for ROW_CAP+1 rows so truncation is detectable', async () => {
    await adminOrdersCsvHandler(makeCtx());
    expect(state.limitArg).toBe(10_001);
  });
});
