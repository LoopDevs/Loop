/**
 * Admin cashback-config upsert handler (ADR 011 / ADR 017 / A2-502).
 *
 * `PUT /api/admin/merchant-cashback-configs/{merchantId}`. Lifted out
 * of `./handler.ts` so the cashback-config write — with its full
 * ADR-017 idempotency-guarded ladder, pre-edit snapshot for the
 * Discord diff, and post-commit audit fanout — sits in its own file
 * separate from the list reader.
 *
 ...
 *
 * Re-exported from `./handler.ts` so `routes/admin-cashback-config.ts`
 * and the test suite keep resolving against the historical path.
 */
