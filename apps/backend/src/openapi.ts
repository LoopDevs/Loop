...
// `LoopAssetCode` + `PayoutState` enums (declared inline in the
// Admin section) so the trustlines + pending-payouts shapes use
// the same definitions across both sections without duplicating
// schema names.
...
const WithdrawalResult = registry.register(
  'WithdrawalResult',
  z.object({
    id: z.string().uuid().openapi({ description: 'credit_transactions.id of the new ledger row.' }),
    payoutId: z
      .string()
      .uuid()
