...
//
// Third backfill batch (#668 scalar merchants, #669 time-series merchants).
// This one covers the per-user axis of the triplet pattern — the same
// three metrics keyed on userId instead of merchantId. Powers the
// /admin/users/:id drill page.
...
const AdminUserFlywheelStats = registry.register(
  ...
  z.object({
    ...
    currency: z.string().length(3).openapi({
      description:
        "Target user's home_currency — both numerator and denominator are scoped to it so the ratio has a coherent denomination.",
    }),
    recycledOrderCount: z.number().int(),
    recycledChargeMinor: z.string().openapi({ description: 'bigint-as-string.' }),
    totalFulfilledCount: z.number().int(),
...
registry.registerPath({
  ...
  description:
    "Admin-scoped mirror of /api/users/me/flywheel-stats. Single LEFT JOIN; 404 on unknown userId (distinguishes 'user not in DB' from 'user with no fulfilled orders', which returns zeroed counts). Home-currency-locked so the ratio has a coherent denomination.",
  tags: ['Admin'],
  security: [{ bearerAuth: [] }],
  ...
  responses: {
    ...
    400: {
      description: 'Malformed userId (not a UUID)',
      content: { 'application/json': { schema: ErrorResponse } },
    },
    404: {
      description: 'User not found',
...
registry.registerPath({
  ...
  description:
    'Admin-scoped per-user sibling of /api/admin/cashback-monthly. 12-month window on credit_transactions of type=cashback. Existence probe separates 404 (unknown userId) from empty entries[] (exists but no cashback in window).',
  tags: ['Admin'],
  security: [{ bearerAuth: [] }],
  request: { params: z.object({ userId: z.string().uuid() }) },
  responses: {
...
registry.registerPath({
  ...
  description:
    'Admin-scoped per-user sibling of #668\'s per-merchant payment-method-share. Default ?state=fulfilled. Zero-filled byMethod across every ORDER_PAYMENT_METHODS value. Support-triage view: "does this user only pay with LOOP?" vs "never touched it?".',
  tags: ['Admin'],
  security: [{ bearerAuth: [] }],
  request: {
    params: z.object({ userId: z.string().uuid() }),
