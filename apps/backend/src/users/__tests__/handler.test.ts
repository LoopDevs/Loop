...
describe('getCashbackSummaryHandler', () => {
  ...
  it('returns lifetime + thisMonth totals in the user home currency', async () => {
    userState.byId = baseUser;
    dbState.executeRows = [{ lifetimeMinor: 4200n, thisMonthMinor: 320n }];
  ...
  it('returns zero totals when the user has no cashback yet', async () => {
    userState.byId = baseUser;
    // SUM over zero rows → null from Postgres; handler COALESCEs, but
    // when the result row itself is undefined (empty result) we fall
    // through to the "0" defaults.
    dbState.executeRows = [];
    const res = await getCashbackSummaryHandler(makeCtx(LOOP_AUTH));
    const body = (await res.json()) as { lifetimeMinor: string; thisMonthMinor: string };
    expect(body.lifetimeMinor).toBe('0');
    expect(body.thisMonthMinor).toBe('0');
  });

  it('handles null aggregates from the driver', async () => {
    userState.byId = baseUser;
    dbState.executeRows = [{ lifetimeMinor: null, thisMonthMinor: null }];
    const res = await getCashbackSummaryHandler(makeCtx(LOOP_AUTH));
    const body = (await res.json()) as { lifetimeMinor: string; thisMonthMinor: string };
    expect(body.lifetimeMinor).toBe('0');
    expect(body.thisMonthMinor).toBe('0');
  ...
  it('handles the {rows: [...]} drizzle result shape', async () => {
    userState.byId = baseUser;
    dbState.executeRows = {
      rows: [{ lifetimeMinor: 100n, thisMonthMinor: 50n }],
    };
    const res = await getCashbackSummaryHandler(makeCtx(LOOP_AUTH));
    const body = (await res.json()) as { lifetimeMinor: string; thisMonthMinor: string };
    expect(body.lifetimeMinor).toBe('100');
    expect(body.thisMonthMinor).toBe('50');
  });
