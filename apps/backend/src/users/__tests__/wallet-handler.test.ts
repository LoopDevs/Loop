...
describe('interestApyBps truthfulness (ADR 031 Phase D)', () => {
  ...
  it('2026-06-15 cold audit v-wallet P0 follow-up: reads 0 when this deployment has no GBPLOOP configured, even with the flag on and a non-zero global APY', async () => {
    // A USD/EUR-only deployment (or one mid-rollout of the GBPLOOP
    // issuer): configuredLoopPayableAssets() never includes GBPLOOP,
    // so the mint worker's ONCHAIN_MINT_ELIGIBLE_ASSETS allowlist can
    // never pay this wallet — the chip must not claim otherwise.
    assetsState.assets = [
      { code: 'USDLOOP', issuer: 'GUSDISSUERXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX' },
    ];
    resolveUserMock.mockResolvedValue(
      activatedUser({ walletAddress: null, walletProvisioning: 'none' }),
    );
