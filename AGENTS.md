...
| `docs/observability.md`                                     | What `/metrics` emits (incl. B-5's new freshness/worker-lead-tick/geo-db/rate-limit-fleet gauges), the bearer-token scrape auth, and `docs/observability/{prometheus.yml,grafana-dashboard.json}` — committed scrape-config-as-code + a dashboard mapping every panel to a `docs/slo.md` target. Closes the 🟢 code/config half of readiness-backlog B-5 / go-live-plan §T1-BS B-5; standing up a real Prometheus/Grafana instance + OTel tracing + a paging tier stays 👤/`[code]` follow-up.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `docs/i18n.md`                                              | i18n mechanics (B-6) — how to add a translatable string (namespace JSON, `useTranslation()`, `<Trans>` for markup-embedded copy, the non-hook `i18n.t()` pattern for `meta()`/`loader()`), how to add a language later (routing, RTL, the SSR-singleton-race gotcha), what NOT to do. Pairs with ADR 043.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `docs/roadmap.md`                                           | What's left for Phase 1, Phase 2, Phase 3                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `docs/thin-currency-promotion.md`                           | Thin-currency promotion cadence (ADR 035) — quarterly review process + the `GET /api/merchants/all?fields=lite` measurement recipe for per-currency/per-country enabled-merchant counts vs the ≥15 threshold, plus the exact promotion checklist (🧭 go/no-go, 💰 order-path wiring if the currency is new, 🟢 `countries.ts` display wiring). Closes the go-live-plan §P3 "thin-currency promotion cadence" checkbox.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| `docs/adr/031-per-currency-yield-architecture.md`           | **Proposed, detailed build-ready 2026-07-10** — LOOPUSD/LOOPEUR are Loop-curated DeFindex vault shares (0% mgmt + 50% perf fee; DeFindex keeps 25% of the fee → Loop nets 37.5% of gains); GBPLOOP is Stellar classic 1:1-backed with **nightly on-chain 3% APY mints**; past-30-day APY + "no guarantee" disclaimer; no yield-source disclosure. **§Detailed design (D1–D10)** added: DeFindex is **deploy-by-config → no from-scratch contract audit**; the wallet provider's job is narrow (custody + `transfer` a Soroban share token — vault `deposit`/`redeem` are operator-signed), so Privy is an assumption + fallback, not a blocker; full Soroban/data-model/invariant/flow spec + build sequence. Remaining for Accepted: signing-path choice, config review + protocol DD, counsel. **Build status:** V1 (schema + read-layer registry) and V2 (`credits/vaults/vault-client.ts` — the Soroban deposit/withdraw/transfer client, verify-before-sign + CF-18 fenced, mock-tested, NOT wired into any flow) shipped; see ADR 049 for V2's dependency decision. |
...
| `docs/adr/046-maplibre-map.md`                              | MapLibre GL JS replaces Leaflet (§P3) — WebGL renderer against the existing CARTO **raster** tiles (no new tile-hosting infra, ships now); self-hosted **vector** tiles (Protomaps PMTiles vs. paid MapTiler/Stadia) deferred to a 👤 operator tile-hosting decision. Server-side `/api/clusters` clustering unchanged; markers/popups/keyboard a11y reimplemented on MapLibre's `Marker`/`Popup` APIs. Drops a stale double-dark-filter CSS bug found in the process.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `docs/adr/047-referral-program.md`                          | **Proposed (design only)** — referral program (§P3): referral codes + a `referral_redemptions` table with a UNIQUE constraint on `referee_user_id` (one reward per referee, ever); the reward is a **conserved credit emission** through the existing primitive guarded by `assert_emission_conservation` (migration 0044 — never a bespoke balance write, so INV-1 holds), triggered by the referee's **first `fulfilled` order** (not signup — the sockpuppet-farming hole); abuse model REUSES ADR 045 (shared-funding-source self-referral block + per-referrer velocity cap + real-purchase gate); whole reward path gated behind `LOOP_PHASE_1_ONLY` (ships dark, activates at the cashback flip). Active build (tables/migration/worker/UI/env-var) + the reward amount (🧭 economics) deferred.                                                                                                                                                                                                                                                                   |
| `docs/adr/048-privacy-analytics.md`                         | First-party, cookieless privacy analytics (§P3) — env-gated (`VITE_ANALYTICS_ENABLED`, default off) `web-vitals`-based Core Web Vitals + a page-view marker, POSTed to `POST /api/public/rum` and folded into `/metrics` (`loop_web_vital_*` histograms, `loop_page_views_total` counter). No cookies, no persistent id, no DB table, no consent banner (GDPR legitimate-interest). Closes the A2-1324 RUM-visibility gap without a third-party vendor.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `docs/adr/049-defindex-vault-client-hand-rolled.md`         | Soroban vault client (ADR 031 §D2/D9, V2) is **hand-rolled on `@stellar/stellar-sdk`** — no `@defindex/sdk` dependency. `@defindex/sdk` was pulled and inspected: it's an HTTP client to DeFindex's _hosted_ API (builds XDR server-side, needs an API key, `number`-typed amounts), not a local transaction builder — unsuitable for a money-moving path. Everything D2's spec needs (`Contract`, `rpc.Server`, `nativeToScVal`/`scValToNative`, `xdr.HostFunction`) is already in the existing `@stellar/stellar-sdk` dependency.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
#   persistent id). Default unset/off — dark by default; flipping it on
#   is a 👤 operator decision, not a code change.
...
# the vault client (credits/vaults/vault-client.ts) uses for account
# loads and simulate/prepare/send/getTransaction calls — a SEPARATE
# JSON-RPC endpoint from LOOP_STELLAR_HORIZON_URL (Horizon is the
# classic-ledger REST API; Soroban RPC is its own thing, even on the
# same network). Required (boot fails) when LOOP_VAULTS_ENABLED=true;
# otherwise nullable.
