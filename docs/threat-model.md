...
| **Single payout submitter is not enforced** (INV-9 residual, A8)                                              | `SKIP LOCKED` + `min_machines_running=1` bounds it; leader election is deferred infra.                                                                                                      | When the fleet routinely runs >1 machine under payout load, or `tx_bad_seq` churn appears in logs.                                        |
...
| **OTP brute-force counter is row-scoped, not identity-scoped** (B5, deferred)                                 | The per-email issuance cap (3/min) + per-IP cap (10/min) bound it; the ceiling semantics are subtle.                                                                                        | Any report of OTP guessing; the correct fix is a per-email failed-attempt counter.                                                        |
