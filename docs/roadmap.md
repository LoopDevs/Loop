...
- [x] ~~**`loopfinance-web` first deploy**~~ — **done**: the web app is deployed and serving at `beta.loopfinance.io` (redeployed to v9 on 2026-07-03, health check passing). The remaining piece is apex/www DNS at public launch — tracked in §Production infrastructure, not orphaned.
...
- [ ] **GeoLite2 refresh cadence** — the `.mmdb` refreshes only on deploys that remember the MaxMind build secrets, and the download is best-effort (a build without the secrets silently falls back to the US default — a quiet degradation of the ADR 034 geo-redirect). _Staleness-signal half done (2026-07-09):_ `/health` now reports `geoDbStale` / `geoDbBuildEpoch`, feeds `softDegradedReasons` (`geo_db_stale`), pages `DISCORD_WEBHOOK_MONITORING` at most once/week (`notifyGeoDbStale`), and logs a boot warn — see `docs/deployment.md` §GeoLite2. The **cadence** half — an operator actually remembering to redeploy with the `--build-secret` flags on a schedule — remains 👤 operator; the new signal just makes a forgotten refresh visible instead of silent. _Note (2026-07-03):_ #1520 added an `Accept-Language` backstop so an empty MaxMind result no longer defaults straight to US, and the planned **Cloudflare migration** (§Production infrastructure) would make MaxMind optional entirely (`CF-IPCountry` replaces it) — if CF lands, this item can be closed rather than solved. Even a fresh GeoLite2 wouldn't have covered the confirmed failing IP; the free DB's ISP gaps are structural.
...
- [x] ~~Deploy web (SSR) to Fly.io~~ — `loopfinance-web` is **deployed and serving** at `beta.loopfinance.io` (+ `loopfinance-web.fly.dev`); redeployed to **v9 on 2026-07-03** (`flyctl status -a loopfinance-web` → machine `started`, health check passing, all routes 200). Apex/www stay parked on GitHub Pages until public launch.
...
- [ ] **Put Cloudflare in front of the Fly apps (the real geolocation fix + launch hardening).** _Why:_ the free MaxMind GeoLite2 DB can't place smaller ISPs — a real UK visitor (Box Broadband, `149.102.9.36`) resolves to an **empty** country and would default to the US. The `Accept-Language` fallback shipped in #1520 is an interim backstop (it leans on the browser's language, not the IP), **not** a real fix. Cloudflare's edge geolocates the same IP correctly (verified 2026-07-03: `cdn-cgi/trace` → `loc=GB`, `colo=LHR`) and adds its `CF-IPCountry` header for free. _Code is already done:_ `home-geo-redirect.tsx` prefers `CF-IPCountry` before MaxMind, so geolocation upgrades automatically the day traffic flows through CF — no further code change. Bonus wins: an EU (LHR) edge fixes the "UK users are SSR'd from `iad`/US" latency, plus WAF / DDoS / CDN caching for launch. _Operator work:_ create the CF account, add the domain, switch the DNS records for `beta`/`api` (and apex at public launch) to **proxied**, then configure so CF does **not** cache the `/` geo-redirect (it's a per-visitor 302) or any authed/`/api` response, and keep the `PRODUCTION_ORIGINS` CORS list + TLS working through the new edge. Also revisit `RATE_LIMIT_MACHINE_COUNT_ESTIMATE` / `TRUST_PROXY` since the client-IP header source changes behind CF. **Full design + the security prerequisite (lock the origin to CF before trusting its headers) + cache rules: `docs/adr/040-cloudflare-edge.md`.**
