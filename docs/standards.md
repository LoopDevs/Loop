...
`// eslint-disable-next-line no-console` with a rationale comment;
don't add new `console.*` calls elsewhere on that basis.
...
- **Auth and payment code requires two-person review** (team convention, not mechanically enforced). Any PR touching `apps/backend/src/auth/`, `apps/backend/src/orders/`, `apps/backend/src/credits/`, `apps/backend/src/payments/` (Stellar payout worker, Horizon reads, price feed), or any Capacitor secure-storage call should get approval from a second engineer who reviews security implications specifically. `.github/CODEOWNERS` auto-requests `@LoopDevs/engineering` on those paths, but the branch-protection ruleset on `main` sets `required_approving_review_count: 0` today so a solo maintainer can land a hot-fix without a round-trip. Once the team grows past one active maintainer, bump the required-approvals count and require code-owner review (see `.github/CODEOWNERS` for the enforced paths).
