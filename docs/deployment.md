...
| `LOOP_VAULTS_ENABLED`                               | No       | `false`            | ADR 031 §D9: vault-subsystem master switch for the LOOPUSD/LOOPEUR DeFindex-vault path (V1 foundation — schema + read layer only). Distinct from `LOOP_PHASE_1_ONLY`. An empty `loop_vaults` registry table + this flag off is byte-identical to pre-migration. |
