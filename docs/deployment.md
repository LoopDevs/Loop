...
| `GIFT_CARD_API_KEY`               | Yes                | —             | Operator API key — boot-required (ADR 052: ctx is the payment processor; authenticates order create + status mirror, catalog/locations scoping, both /ws topics)                                                                                                                                                                      |
| `GIFT_CARD_API_SECRET`            | Yes                | —             | Operator API secret (pairs with `GIFT_CARD_API_KEY`)                                                                                                                                                                                                                                                                                  |
...
| `LOOP_PHASE_1_ONLY`                       | No                 | `false` | Phase 1 launch gate. `true` hides every Phase 2+ web surface (cashback navbar links, `/settings/wallet`, `/settings/cashback`, `/cashback`, onboarding currency picker + wallet-intro, "you've earned X" copy); discount badges stay. UI-side equivalent of the backend Phase 2 gates (Stellar secrets/issuers / `LOOP_AUTH_NATIVE_ENABLED` / `INTEREST_APY_BASIS_POINTS` — keep those off too). Flip back to `false` to launch cashback — server-side only, no app-store resubmission. |
...
| `LOOP_VAULT_APY_SNAPSHOT_INTERVAL_HOURS`    | No          | `24`             | ADR 031 §D8, V5b: cadence of the APY snapshot cron (`credits/vaults/vault-apy-snapshot.ts`) that records each active vault's live share price into `vault_share_price_snapshots`, feeding `GET /api/me/vault-apy`. Runs under `LOOP_VAULTS_ENABLED`.                                                                                                                                                            |
...
| `LOOP_AUTH_ROW_PURGE_INTERVAL_HOURS`        | No          | `1`              | CF-26 / X-PRIV-07/08: cadence of the always-on auth-row retention purge sweep. Runbook: `docs/runbooks/dsr.md`.                                                                                                                                                                                                                                                                                                 |
...
| Variable                | Required | Default | Description                                                                                            |
| ----------------------- | -------- | ------- | ------------------------------------------------------------------------------------------------------ |
| `LOOP_KILL_ORDERS`      | No       | `false` | Gates `POST /api/orders/loop`.                                                                         |
| `LOOP_KILL_ORDERS_LOOP` | No       | unset   | Same path. Set → overrides `LOOP_KILL_ORDERS`; unset → falls back.                                     |
