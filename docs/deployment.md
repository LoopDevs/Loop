...
| `APPLE_TEAM_ID`                   | No                 | —             | M-3 deep linking: gates `GET /.well-known/apple-app-site-association` — 404 `WELL_KNOWN_NOT_CONFIGURED` while unset. Fill in after Apple Developer Program enrollment (go-live-plan L1-4).                                                                                                                                            |
| `ANDROID_CERT_SHA256`             | No                 | —             | M-3 deep linking: gates `GET /.well-known/assetlinks.json` — 404 `WELL_KNOWN_NOT_CONFIGURED` while unset. Comma-separated SHA-256 cert fingerprints (debug + release side by side during rollout). Fill in after the release keystore is created (go-live-plan L1-5).                                                                 |
...
| `NODE_ENV`                        | No                 | `development` | Environment. Schema default is `development`; prod deployments set it to `production` explicitly (our `fly.toml` + Dockerfile both do). Several production-only guards (step-up boot guard, image-proxy resolved-URL validation, prod-warns) key off `NODE_ENV === 'production'`, so leaving it unset in prod silently disables them. |
...
| `LOOP_JWT_RSA_PRIVATE_KEY`                | No                 | —       | ADR-030 Phase A: PKCS8 PEM RSA private key. When set, new Loop JWTs sign RS256 (RFC 7638 `kid` header) and the public keys publish at `GET /.well-known/jwks.json` for external verifiers (wallet provider custom auth). Unset → HS256 signing continues. Generate with `openssl genpkey -algorithm RSA -pkeyopt rsa_keygen_bits:2048`; boot fails on malformed / non-RSA PEM. Cutover procedure: `docs/runbooks/jwt-key-rotation.md`.                                                 |
| `LOOP_JWT_RSA_PRIVATE_KEY_PREVIOUS`       | No                 | —       | Prior RSA key accepted during rotation; its public half stays published in the JWKS. Remove after all outstanding tokens expire (30 days + the 1h JWKS cache window).                                                                                                                                                                                                                                                                                                                  |
| `LOOP_AUTH_NATIVE_ENABLED`                | Yes (prod)         | `false` | Gates the Loop-native OTP path. `false` → legacy CTX-proxy. **R3-7: production boot fails unless this is `true`** so an unset flag cannot silently revert auth to CTX.                                                                                                                                                                                                                                                                                                                 |
| `DISABLE_NATIVE_AUTH_ENFORCEMENT`         | No                 | —       | Emergency rollback / staging override for the R3-7 production guard. Only exact value `1` is accepted. Do not set for normal production.                                                                                                                                                                                                                                                                                                                                               |
...
| `LOOP_ADMIN_STEP_UP_SIGNING_KEY`          | Yes (prod)         | —       | ADR-028 / A4-063: 32+ char key signing the 5-min `X-Admin-Step-Up` JWTs. **Hardening B3: production boot fails without it** (every destructive admin write would 503 `STEP_UP_UNAVAILABLE`) unless `DISABLE_ADMIN_STEP_UP_ENFORCEMENT=1` deliberately ships the surface disabled. Keep separate from `LOOP_JWT_SIGNING_KEY` so a JWT-key compromise doesn't widen to step-up.                                                                                                          |
...
| `ADMIN_EMAILS`                        | Yes (native)       | `''`           | CF-30: comma-separated verified emails granted admin on the Loop-native path (ADR 013). Required to reach `/api/admin/*` when `LOOP_AUTH_NATIVE_ENABLED=true` — `ADMIN_CTX_USER_IDS` only covers CTX-anchored users. Case-insensitive; granted only on a verified email.                                                                                                                                                                                                                                                                 |
...
| `ADMIN_DAILY_WITHDRAWAL_CAP_MINOR`    | No                 | `100_000_000`  | Same shape as above, for admin **emissions** (ADM-01, 2026-06-30 cold audit; the withdrawal writer it originally capped was re-scoped to emission by ADR 036 — hardening A1 revived the orphaned cap for the same value-leaves-the-system surface). 0 disables.                                                                                                                                                                                                                                                                          |
| `LOOP_ORDER_VELOCITY_MAX_PER_WINDOW`  | No                 | `20`           | ADR 045 (B-3): max orders per USER per rolling window on `POST /api/orders/loop`. Over → 429 `ORDER_VELOCITY_EXCEEDED`. 0 disables the count dimension.                                                                                                                                                                                                                                                                                                                                                                                  |
| `LOOP_ORDER_VELOCITY_WINDOW_HOURS`    | No                 | `24`           | ADR 045 (B-3): rolling window size (hours) for both velocity dimensions.                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `LOOP_ORDER_VELOCITY_MAX_VALUE_MINOR` | No                 | `500_000`      | ADR 045 (B-3): max summed `charge_minor` per user per charge-currency per window. 0 disables the value dimension.                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| `RATE_LIMIT_MACHINE_COUNT_ESTIMATE`   | Recommended (prod) | `1`            | CF2-10 (2026-06-30 cold audit) stopgap, **now the fallback only** (S4-4, see §Rate-limiter fleet-size estimate below): `rateLimitMap` is in-memory and per-machine, so every configured per-route budget is actually `max × (live Fly machine count)`. A live `.internal` DNS-derived estimate is preferred whenever fresh; this var is used only when `FLY_APP_NAME` is unset or DNS has failed past its grace period. Defaults to 1 (no division, same posture as `TRUST_PROXY`) — set explicitly in production as the fallback floor. |
| `FLY_APP_NAME`                        | Auto (Fly)         | —              | S4-4: Fly injects this into every Machine's runtime automatically — never set by hand. Names the app's private `.internal` DNS zone the fleet-size estimator queries.                                                                                                                                                                                                                                                                                                                                                                    |
...
| `LOOP_STELLAR_USDC_ISSUER`              | Yes (prod)  | —                                                  | Circle USDC issuer G-account. AUDIT-2 finding A: Stellar asset codes aren't unique, so the payment watcher's issuer-match guard (`payments/horizon.ts`) fails CLOSED without it — matches NO USDC deposit, never "any issuer" (an unpinned issuer previously let an attacker's self-issued fake "USDC" mark a real order paid). **Production boot fails without it** (mirrors `LOOP_ADMIN_STEP_UP_SIGNING_KEY`) unless `DISABLE_USDC_ISSUER_ENFORCEMENT=1` deliberately ships the USDC rail disabled. |
| `DISABLE_USDC_ISSUER_ENFORCEMENT`       | No          | —                                                  | Emergency opt-out for the production `LOOP_STELLAR_USDC_ISSUER` boot guard above. Only exact value `1` is accepted. Do not set for normal production — every USDC deposit will silently fail to match.                                                                                                                                                                                                                                                                                                |
...
| `LOOP_VAULTS_ENABLED`                               | No          | `false`            | ADR 031 §D9: vault-subsystem master switch for the LOOPUSD/LOOPEUR DeFindex-vault path. V1 shipped schema + read layer; V2 adds the Soroban deposit/withdraw/transfer client (not wired into any flow yet). Distinct from `LOOP_PHASE_1_ONLY`. Requires `LOOP_SOROBAN_RPC_URL` to also be set — boot fails otherwise. An empty `loop_vaults` registry table + this flag off is byte-identical to pre-migration. |
| `LOOP_SOROBAN_RPC_URL`                              | Conditional | unset              | ADR 031 §D2/D9, V2: Soroban RPC endpoint for the vault client's account loads + simulate/prepare/send/getTransaction calls — distinct from `LOOP_STELLAR_HORIZON_URL` (Horizon is the classic-ledger REST API). Required when `LOOP_VAULTS_ENABLED=true`.                                                                                                                                                       |
| `LOOP_VAULT_APY_SNAPSHOT_INTERVAL_HOURS`            | No          | `24`               | ADR 031 §D8, V5b: cadence of the APY snapshot cron (`credits/vaults/vault-apy-snapshot.ts`) that records each active vault's live share price into `vault_share_price_snapshots`, feeding `GET /api/me/vault-apy`. Runs under `LOOP_WORKERS_ENABLED` AND `LOOP_VAULTS_ENABLED`.                                                                                                                                 |
...
| `LOOP_CTX_PAYMENT_MAX_BPS_OF_EXPECTED`              | No          | `12500`            | R3-5: upper-band sanity check before paying CTX. Refuses a SEP-7 amount above this many basis points of the expected wholesale XLM quote.                                                                                                                                                                                                                                                                       |
...
| `LOOP_OPERATOR_FLOAT_RECONCILIATION_INTERVAL_HOURS` | No          | `24`               | R3-1: cadence of the operator XLM/USDC float reconciler. Runs under `LOOP_WORKERS_ENABLED`; fails closed to `needs_baseline` (paged) until an operator creates a baseline. Runbook: `docs/runbooks/operator-float-drift.md`.                                                                                                                                                                                    |
| `LOOP_OPERATOR_FLOAT_XLM_THRESHOLD_STROOPS`         | No          | `10000000` (1 XLM) | R3-1: XLM drift-paging threshold, stroops. Wider than USDC's to absorb Stellar tx fees the reconciliation model doesn't count.                                                                                                                                                                                                                                                                                  |
| `LOOP_OPERATOR_FLOAT_USDC_THRESHOLD_STROOPS`        | No          | `1`                | R3-1: USDC drift-paging threshold, stroops. USDC pays no on-chain fee, so the default stays effectively exact.                                                                                                                                                                                                                                                                                                  |
...
| `LOOP_KILL_EMISSIONS`     | No       | `false` | Gates the admin emission (ADR 036, ex-withdrawal) + compensation endpoints.                               |
...
#### Rate-limiter fleet-size estimate (S4-4, docs/readiness-backlog-2026-07-03.md)

The per-IP rate limiter (`apps/backend/src/middleware/rate-limit.ts`) is an in-memory,
per-machine `Map`, so every configured per-route budget is actually `max × (live Fly
machine count)` — see AGENTS.md's middleware-stack section, item 6. `fly.toml` sets
`auto_start_machines = true`, so a **static** machine-count divisor
(`RATE_LIMIT_MACHINE_COUNT_ESTIMATE`) goes stale the moment the fleet scales, and goes
wrong in the unsafe direction (limits too LOOSE) exactly under the load spike you'd want
them tight for.

`apps/backend/src/middleware/fleet-size.ts` fixes this with a **dynamic** estimate
instead of a shared store: Fly gives every app a private `.internal` DNS zone —
`<FLY_APP_NAME>.internal` resolves to one AAAA (IPv6 6PN) record per currently-started
Machine belonging to that app, fleet-wide across regions; stopped machines are excluded
by the platform automatically. `FLY_APP_NAME` is injected into every Machine's runtime
automatically. A background interval (never on the request path) re-queries this every
30s via Node's stock `dns.promises.resolve6` — no custom resolver configuration is
needed inside a Fly Machine (`/etc/resolv.conf` is preconfigured to the platform's
internal resolver at boot).

**Why not a shared counter (Redis/Postgres).** Deliberately rejected: it adds a hot-path
round-trip to every rate-limited request, and turns a volumetric flood into a database
write storm — worse than today's inaccuracy, not better. `docs/adr/040-cloudflare-edge.md`'s
planned Cloudflare edge is the eventual durable answer (a single edge-side limiter ahead
of the whole fleet); this is the interim fix for the in-memory-per-machine design in the
meantime.

**Failure handling.** A DNS failure must never make limits looser than the static
estimate. On a refresh failure the last-known-good dynamic value keeps being served for
5 minutes (rides out a transient DNS blip); only past that grace period does the
estimator fall back to `RATE_LIMIT_MACHINE_COUNT_ESTIMATE`. `FLY_APP_NAME` unset (local
dev, CI, non-Fly hosts) skips DNS entirely and always uses the static fallback — no
behaviour change from before this fix.

`/health` reports the currently-effective value: `rateLimitFleetEstimate` (the number)
and `rateLimitFleetEstimateSource` (`'dynamic'` when a fresh DNS read is in effect,
`'static'` when on the fallback). Purely informational — neither field affects
`softDegraded`/`criticalDegraded`/the HTTP status.
...
| `LOOP_E2E_REFRESH_TOKEN`      | secret   | `e2e-real.yml` (real Tranche-1 purchase)          | Weekly `schedule:` run (Q6-7) skips cleanly with a `::notice::` log line (`preflight` job); a manual `workflow_dispatch` run is unaffected by the skip and still hard-fails at auth like before. Bootstrap + rotate via `scripts/bootstrap-e2e-refresh-token.sh` (CTX rotates the refresh token on every use; the workflow re-uploads the new one). |
| `STELLAR_TEST_SECRET_KEY`     | secret   | `e2e-real.yml`                                    | Same graceful skip on the weekly `schedule:` run; a manual run still hard-fails — cannot pay the order. Mainnet test wallet secret — fund with a few XLM; treat as production-adjacent (it holds real funds).                                                                                                                                       |
| `DISCORD_WEBHOOK_MONITORING`  | secret   | `audit-cron.yml`, `e2e-real.yml` (failure notify) | No Discord alert on a failed weekly npm-audit or real-chain e2e run. The failure is still visible in the Actions tab (job goes red) — just not proactively surfaced.                                                                                                                                                                                |
