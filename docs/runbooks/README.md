...
| **Peg correctness**   | [peg-break-on-fulfillment.md](./peg-break-on-fulfillment.md) — `notifyPegBreakOnFulfillment` — order charge-ccy ≠ user home-ccy at fulfillment; off-chain credited, on-chain payout skipped, manual compensation needed (A4-023) |
| **Treasury floor**    | [usdc-below-floor.md](./usdc-below-floor.md) — `notifyUsdcBelowFloor` reserve-floor alert                                                                                                                                        |
| **Interest pool**     | [interest-pool-low.md](./interest-pool-low.md) — `notifyInterestPoolLow` — forward-mint pool below `LOOP_INTEREST_POOL_MIN_DAYS_COVER`; mint the next batch (paired `notifyInterestPoolRecovered`)                               |
...
|                       | [deposit-skip-recorded.md](./deposit-skip-recorded.md) — `notifyDepositSkipRecorded` — first-touch skip alert (`missing_credit_row` / `processing_error`); deposit safe, sweep retries                                           |
