...
| A2-116 | Low          | ~~`pr-automation.yml`, `pr-review.yml` lack top-level `permissions`~~ **resolved-pending-review**: both workflows now carry `permissions: {}` at the workflow level with per-job narrow grants (`contents: read`, `pull-requests: write`). `A2-116:` marker present in both files as the source comment explaining the deny-by-default stance. |
...
| A2-411 | Low      | ~~`.husky/pre-push` doesn't call `scripts/verify.sh`; skips typecheck/lint/format:check~~ **resolved-pending-review**: `.husky/pre-push` carries the `A2-411:` marker and invokes `./scripts/verify.sh` (typecheck + lint + format:check + lint-docs + tests) right after the branch-name convention gate. CI's Quality job now has local-fail parity. |
