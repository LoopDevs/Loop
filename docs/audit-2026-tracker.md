...
**Medium (7):** ~~A2-1602 Admin CSV exporters don't escape leading `=`/`+`/`-`/`@` (spreadsheet formula injection)~~ **resolved-pending-review** (shared `apps/backend/src/admin/csv-escape.ts` applied across 18 exporters) · A2-1603 Circuit breaker tripped by attacker-induced upstream 5xx denies 30s of legit auth traffic · ~~A2-1605 `DISABLE_RATE_LIMITING` env var has no prod-boot guard~~ **resolved-pending-review** (parseEnv throws on prod + DISABLE*RATE_LIMITING=true) · A2-1606 `/metrics` unauth + unrate-limited; leaks route map + live circuit state *[in flight #903]_ · A2-1607 `/openapi.json` unauth; exposes 97 admin routes + schemas _[in flight #903]\_ · A2-1610 No per-admin-per-day magnitude cap on credit adjustments beyond ±10M minor per-request · ~~A2-1612 `ConsoleEmailProvider` logs raw OTP; if `@sentry/pino` flipped on, OTPs land in Sentry before Pino redacts~~ **resolved-pending-review** (raw code only logged when `SENTRY_DSN` is unset; when set, code is replaced with `[REDACTED: SENTRY_DSN set]` + the log message directs devs to the DB).
