...
**Low (7):** ~~A2-1511 `upstreamUrl` path unrestricted~~ **resolved-pending-review**: `apps/backend/src/upstream.ts:19-48` rejects leading `//` (protocol-relative), control chars (C0 + DEL + C1), literal `..` segments, and percent-encoded `%2e%2e` — defense-in-depth layer over caller-side input validation. · A2-1514 memo-length constants duplicated · A2-1515 Discord test-ping clean _(no-action — audit noted Discord test-pings reveal no unexpected state)_ · A2-1516 Discord vs web symbol drift for exotic currencies · A2-1524 canonical `formatMinorCurrency` adoption 4/29 sites · A2-1527 `IMAGE_PROXY_ALLOWED_HOSTS` only caught at prod boot _(design choice: parseEnv throws at boot per audit A-025)_ · A2-1528 no CI docker-image smoke with prod env.
