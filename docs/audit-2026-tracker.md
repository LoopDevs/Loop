...
| A2-555 | Info         | ~~Upstream 500-char body logs could leak JWT-shaped strings~~ **resolved-pending-review**: `scrubUpstreamBody()` in `apps/backend/src/upstream-body-scrub.ts` redacts JWT-shape tokens before they land in the Pino error log; paired test at `__tests__/upstream-body-scrub.test.ts` pins the redaction contract.                                             |
...
| A2-560 | Low          | ~~OTP attempts ceiling off-by-one (`lte` vs `lt`)~~ **resolved-pending-review**: `auth/otps.ts:103` uses strict `lt` with the `A2-560:` marker — `OTP_MAX_ATTEMPTS` is now the true ceiling (the prior `lte` let attempts reach MAX before locking out).                                                                                                       |
| A2-561 | Low          | ~~`incrementOtpAttempts` bumps every live row for the email~~ **resolved-pending-review**: `auth/otps.ts:128` now targets the single newest live row with the `A2-561:` marker; the prior multi-row bump could cause a race where one bad guess burned through unrelated OTP tries.                                                                            |
| A2-562 | Low          | ~~`revokeAllRefreshTokensForUser` exported/tested but not wired to any route~~ **resolved-pending-review**: `auth/native.ts:235` calls `revokeAllRefreshTokensForUser(record.userId)` in the reuse-detection branch of `nativeRefreshHandler` — a replayed revoked refresh triggers a family-wide revoke so a stolen token lineage dies.                       |
...
| A2-567 | Medium       | ~~Google `iss` exact-match rejects valid `accounts.google.com` (no `https://`) variants~~ **resolved-pending-review**: `auth/social.ts:198` + `auth/id-token.ts:97` accept any member of `expectedIssuers: ['https://accounts.google.com', 'accounts.google.com']`; paired test at `__tests__/id-token.test.ts:203` pins the two-variant behaviour explicitly. |
