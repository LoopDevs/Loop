...
| A2-1101 | ~~Admin UI shell renders for any authenticated user; only `admin._index.tsx` surfaces the 401/403 banner — others flash `<AdminNav/>` first~~ **partially resolved**: new `<RequireAdmin>` wrapper (`apps/web/app/components/features/admin/RequireAdmin.tsx`) gates the shell on `/api/users/me.isAdmin` — non-admins see the banner with no nav flash. Wired into `admin._index`, `admin.audit`, `admin.cashback`; remaining 14 admin routes follow up via the same mechanical wrap pattern. |
