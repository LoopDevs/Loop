...
### E2E (`tests/e2e` + `tests/e2e-mocked` + `tests/e2e-flywheel` + `tests/e2e-loop-purchase` + `tests/e2e-admin-support`)
...
- `tests/e2e-admin-support/admin-dashboard.test.ts` (Q6-5, `docs/money-auth-worklist.md`) — the **admin/support dashboard** smoke: the biggest admin-surface gap this session (A5-1 order re-drive, A5-2 revoke-sessions, A5-3 login/OTP support tooling, A5-4 order refund, A5-6 stuck-orders visibility, A5-7 audit timeline, A5-8 ledger browser) had unit + staff-gating + component coverage but nothing drove it through a real browser as an authenticated staff user. Two tests, own config/port range (`playwright.admin.config.ts`) with its own `staff_roles` seed (an admin-tier user, a support-tier user, and a "customer" user with a stuck `paid` order, a `fulfilled` order + cashback ledger row, an OTP-lockout row, and a live refresh-token row — all fixed ids in `global-setup.ts` so the suite navigates straight to `/admin/orders/:id` / `/admin/users/:id`): **admin** — dashboard loads, stuck-orders (A5-6), the ledger browser with a filter + pagination affordance (A5-8), the orders list, and a user-360 page (A5-7 audit timeline + A5-3 auth-state) all render real seeded data; the order re-drive (A5-1) and refund (A5-4) write affordances render and trip the ADR 028 step-up gate on submit (never completed — both are real money-moving writes, so the assertion is "the gate triggers", not "procurement succeeds"); revoke-sessions (A5-2, which has no step-up by design) is completed end-to-end as the one safe write, with a post-reload check that the audit timeline picked up the resulting `session_revoked` row. **support** — the same read surfaces render, but every admin-only write affordance and nav/page (e.g. `/admin/staff`) is absent or denied. Authenticates via the same test-only `/__test__/mint-loop-token` endpoint as the flywheel suite (grants a session, not a role — the `staff_roles` grants are a direct SQL seed). Runs as a third step in the CI `test-e2e-flywheel` job.
...
npm run test:e2e:admin           # admin/support dashboard smoke (Q6-5)
...
to fail CI on a real regression.

Q6-5 (2026-07-10, admin/support UI E2E — `docs/money-auth-worklist.md`)
landed as pure Playwright/e2e work — no vitest component tests, so no
coverage-floor movement to ratchet. Re-measured for confirmation at the
same commit: statements 63.63% / branches 59.65% / functions 62.32% /
lines 65.13% — within noise of the Q6-8 baseline above, not a Q6-5
effect (Playwright suites run against a live server, not
instrumented code, so they never move this number).
