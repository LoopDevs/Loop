...
| `PAYMENT_METHOD_DISABLED`  | 400    | `POST /api/orders/loop` with `paymentMethod=credit` (A4-110b — credit-source bucketing not yet shipped) | Terminal for now. The credit payment method is gated off until `user_credits` gains a source-tag column. Offer XLM / USDC instead. |
...
| `STEP_UP_REQUIRED`         | 401    | ADR-028 / A4-063 admin step-up gate (credit-adjust / withdrawals / payout-retry) when the `X-Admin-Step-Up` header is missing         | Terminal until step-up. The admin UI prompts for the step-up (password) modal, obtains a 5-min token, then retries with the header.      |
| `STEP_UP_INVALID`          | 401    | Same gate when the `X-Admin-Step-Up` token is expired or its signature fails                                                          | Retryable after a fresh step-up. The UI re-prompts for the step-up modal (token expired / invalid), then retries.                        |
| `STEP_UP_SUBJECT_MISMATCH` | 401    | Same gate when the step-up token's `sub` differs from the authenticated admin (replayed token from another admin)                     | Terminal. Log the admin out — a step-up token issued for a different admin was presented. Do not retry.                                  |
| `USER_NOT_FOUND`           | 404    | `PUT /api/admin/users/:userId/home-currency` (admin home-currency change) when the target user doesn't exist                          | Terminal. The `userId` doesn't resolve. Fix the id; don't retry.                                                                         |
| `MERCHANT_NOT_FOUND`       | 404    | `POST /api/users/me/favorites` when the merchant id isn't in the catalog                                                              | Terminal. The merchant id isn't in the catalog (evicted or never existed). Refresh the catalog list before retrying.                     |
...
| `HOME_CURRENCY_UNCHANGED`             | 409    | `PUT /api/admin/users/:userId/home-currency` when the requested currency already matches the user's current one | Terminal. No-op — the user is already on that currency. Don't retry.                                                                          |
| `HOME_CURRENCY_HAS_LIVE_BALANCE`      | 409    | Same endpoint when the user holds a non-zero balance in the old currency (changing it would strand funds)       | Terminal until the balance is zeroed (spent / withdrawn). Surface the blocking balance; a change can't proceed while funds are live.          |
| `HOME_CURRENCY_HAS_IN_FLIGHT_PAYOUTS` | 409    | Same endpoint when the user has pending / submitted cashback payouts in the old currency                        | Terminal until those payouts settle. Retry after the in-flight payouts reach `confirmed`.                                                     |
| `CONCURRENT_CHANGE`                   | 409    | Same endpoint when an optimistic-concurrency check loses (a competing home-currency write landed first)         | Retryable. Re-read the user's current currency and re-issue the change against the fresh state.                                               |
| `FAVORITES_LIMIT_EXCEEDED`            | 409    | `POST /api/users/me/favorites` when the user already has the maximum pinned merchants                           | Terminal until the user removes a favourite. Surface the cap; prompt them to unpin one before adding another.                                 |
| `WEBHOOK_NOT_CONFIGURED`              | 409    | `POST /api/admin/discord/test` when the selected channel's webhook env var is unset                             | Terminal until ops sets the webhook URL for that channel. Surface which channel is unconfigured; retrying the test won't help until then.     |
...
| `STEP_UP_UNAVAILABLE`          | 503    | ADR-028 / A4-063 admin step-up gate when `LOOP_ADMIN_STEP_UP_SIGNING_KEY` is unset on the deployment (the gate fails closed)                                              | Retryable after ops generates `LOOP_ADMIN_STEP_UP_SIGNING_KEY` and redeploys. Not a user-input problem; surface "admin step-up not configured — contact ops".                                                                             |
