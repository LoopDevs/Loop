...
| `INSUFFICIENT_BALANCE`     | 400    | `POST /api/admin/users/:userId/emissions` (ADR-024/036) | Terminal. Admin attempted an emission larger than the user's mirror balance (would mint unbacked LOOP) — message carries available vs requested. |
...
| `SUBSYSTEM_DISABLED`           | 503    | Runtime kill switch on `auth`, `orders`, or `emissions` surfaces                                                                                                          | Retryable only after operators reopen the subsystem. Honour `Retry-After` when present.                                                                                                                                                   |
