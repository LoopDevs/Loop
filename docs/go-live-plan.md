...
- [ ] **Admin/support tooling (A5):** ✅ A5-1 order re-drive (biggest hole) — shipped 2026-07-09, review-first PR #1609 open (not yet merged): `POST /api/admin/orders/:orderId/redrive`, admin + step-up (`order-redrive` scope), reuses `procureOne`/`ctx_settlements` idempotency, no new money logic; cancel-and-refund explicitly deferred to A5-4 · A5-4 order-bound refund UI+policy · A5-6 stuck-orders/payouts visibility · A5-7 per-subject audit view · A5-8 fleet-wide ledger browser · A5-9 bulk + drift-correction · A5-2 session-revocation UI 🔐 · A5-3 login/OTP support tooling 🔐 · A5-5 operator-mediated DSR. (all 💰 unless tagged)
