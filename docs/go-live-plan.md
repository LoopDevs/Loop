...
- [x] **Fraud/abuse (B-3):** ✅ Phase 1 shipped 2026-07-10, review-first PR open (not yet merged): ADR 045 design + per-user order-create velocity limit (`fraud/velocity.ts`, bounded/indexed, fail-closed, count + per-currency value dimensions) + duplicate-account shared-funding-source detection (`fraud/duplicate-account-signals.ts`, flag-only, `fraud_signals` table + Discord page, never auto-block). Chargeback handling scoped as a documented T3/Plaid-card hook (no chargeback threat exists on Phase-1 crypto-only funding); device/IP dup-account capture and an admin UI for `fraud_signals` explicitly deferred (see ADR 045 Consequences). 💰 + design/ADR.
