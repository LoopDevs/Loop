...
- [ ] **Scale/fleet (S4):** ✅ S4-7 catalog fetch trim (done) · ✅ S4-6 bound admin ledger-drift scan (done) · ✅ S4-2 wallet-provisioning fleet-lock (done) · ✅ S4-3 single-flight interest-mint reads (done) · ✅ S4-8 dedupe per-machine watchers/alerts (done 2026-07-09). S4-1 payout throughput ceiling (L, architectural) 💰 · S4-4 rate-limiter shared store 🟢 · S4-5 raise DB pool / plan PgBouncer 👤+🟢.
