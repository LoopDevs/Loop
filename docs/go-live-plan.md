...
>
> Snapshot: **2026-07-10.** Tick here for the master view.
...
- [x] **T0-3 · Money-invariant DB layer as a required merge check.** 💰 + 👤 **Done 2026-07-10 (#1614):** `scripts/check-money-invariants.mjs` statically asserts every "DB:"-tier money object (`assert_emission_conservation` trigger, payout/settlement/ledger unique indexes, ledger/order CHECKs) still exists with its documented shape; wired into the REQUIRED Quality CI job so a diff that drops one fails CI even if it also updates `schema.ts`.
...
- [ ] **Scale/fleet (S4):** ✅ S4-7 catalog fetch trim (done) · ✅ S4-6 bound admin ledger-drift scan (done) · ✅ S4-2 wallet-provisioning fleet-lock (done) · ✅ S4-3 single-flight interest-mint reads (done) · ✅ S4-8 dedupe per-machine watchers/alerts (done 2026-07-09) · ✅ S4-4 rate-limiter dynamic fleet-size estimate (implemented 2026-07-09 — dynamic `.internal`-DNS estimator, not the shared-store option, which was deliberately rejected as hot-path-costly; PR open, 🔐 auth-review pending merge — rate limiting is an auth-review surface, not 🟢 self-merge). S4-1 payout throughput ceiling (L, architectural) 💰 — **2026-07-10: code shipped** (merged 2026-07-10): `docs/adr/044-payout-throughput.md` picks Stellar **channel accounts** (N pre-funded sequence-owning accounts, op-level `source` override keeps the operator/issuer as the real funder) over operator/issuer account sharding or batching; `channel-accounts.ts` + `submitPayout`'s additive `channelSecret` + `payout-worker.ts` shard-and-`Promise.all` dispatch, A8 fleet-wide leader lock unchanged, `LOOP_STELLAR_PAYOUT_CHANNEL_SECRETS` unset (default) = byte-identical pre-ADR-044 behaviour. **Still open (👤):** operator provisions + funds N channel accounts and confirms real throughput scaling before this ticks fully done · S4-5 raise DB pool / plan PgBouncer 👤+🟢 **2026-07-09: docs half done** — `docs/deployment.md` §"Database pool sizing & PgBouncer" has the verified pool-vs-concurrency gap, the sizing formula, a PROPOSED ~25/machine starting point, and the full ⚠️ PgBouncer/session-advisory-lock writeup (every `withAdvisoryLock` call site vs. the unaffected transaction-scoped locks; readiness-backlog S4-5). **Still open (👤):** operator reads the live Postgres `max_connections` and sets `DATABASE_POOL_MAX` against it.
- [ ] **Admin/support tooling (A5):** ✅ A5-1 order re-drive (biggest hole) — shipped 2026-07-09 **paid-only** after money-review, merged 2026-07-10 (#1609): `POST /api/admin/orders/:orderId/redrive`, admin + step-up (`order-redrive` scope), reuses `procureOne`/`ctx_settlements` idempotency, no new money logic; `procuring` refused (`ORDER_REDRIVE_IN_PROGRESS` — double-pay/stranding risk, the recovery sweep owns those); cancel-and-refund deferred to A5-4 · ✅ A5-4 order-bound refund UI+policy — shipped + merged 2026-07-10: new `POST /api/admin/orders/:orderId/refund`, admin + step-up (`order-refund` scope), ADR-017 envelope. Operator-decided policy: paid/procuring/failed refund directly; **fulfilled** refundable ONLY behind a required code-unused attestation (`{codeUnused,attestationNote}`) — the compensating control for the CTX-redemption-verification Loop lacks. No new money path — dispatches to the EXISTING primitives per payment_method (xlm/usdc → `applyOrderAutoRefund`→A6 `refundDeposit` on-chain-to-sender; credit → `applyAdminRefund` mirror; **loop_asset fails closed** per R3-2). paid/procuring fenced to `failed` first; procuring-with-CTX-paid refused (`loopPaidCtx`). INV-8-idempotent (rides the migration-0013 index + deposit-refund claim guard). Web `RefundOrderPanel` with the fulfilled-order double-spend warning + required attestation checkbox. Double-spend risk operator-accepted (attestation+audit) — recorded in `docs/threat-model.md` · ✅ A5-6 stuck-orders/payouts support-visibility — shipped + merged 2026-07-10: the backend read was already support-tier (ADR 037 blanket rider, since PR #1430); fixed the web-side mismatch (`admin.stuck-orders.tsx` was `RequireAdmin`, dashboard card was `isAdminRole`-gated) plus a regression pin that the triage default fires before the procurement-sweep terminal cutoff · ✅ A5-7 per-subject audit view — shipped + merged 2026-07-10 (#1622): `GET /api/admin/users/:userId/audit` merges admin actions targeting the user + credit_transactions + orders + payouts + refresh_tokens revocations + an OTP-lock snapshot into one newest-first timeline; support-tier, read-only, per-source bounded (`?limit` default 8/[1,20], default response 41 events — under CF-10's 50-row bulk-read threshold) with PER-SOURCE COMPOUND `(timestamp,id)` keyset cursors (not one shared cursor and not a bare timestamp — two independent-review P1s: a global cursor drops rows from denser sources under uneven density, and a bare-timestamp cursor drops rows when >limit share one timestamp, e.g. a mass session revoke stamping one `revokedAt`); `UserAuditTimeline.tsx` on `/admin/users/:userId` drill-links each row + accumulates pages; also closed a tracked A5-8 P2 (ledger's default 50-row page equalled the CF-10 threshold and tripped it every load — fixed via a per-path override) · ✅ A5-8 fleet-wide ledger browser — shipped + merged 2026-07-10 (#1620): `GET /api/admin/ledger`, support-tier, read-only, keyset-paginated (bounded, indexed — new `credit_transactions_created_at` index, migration 0058, mirrors S4-6/PERF-005), filterable by userId/type/reference/date-range; `/admin/ledger` web page + drill-links both ways with `/admin/users/:userId` · ✅ A5-9 drift-correction — RESOLVED 2026-07-10 (operator decision, recorded with A5-4): NO new drift-write primitive; drift is corrected via the EXISTING conservation-trigger-guarded per-user credit-adjust (step-up + reason + audit), driven by the A5-8 ledger browser + R3-1 reconciliation tools (procedure in `docs/runbooks/ledger-drift.md` + `operator-float-drift.md`); bulk-selection UX deferred · ✅ A5-2 session-revocation UI 🔐 — shipped + merged 2026-07-10: `RevokeSessionsPanel` on `/admin/users/:userId` wires the existing (previously orphaned) `POST /api/admin/users/:userId/revoke-sessions`; admin-gated, plain confirm dialog (the endpoint predates the ADR 017 envelope — no reason/idempotency/step-up on its own contract, so the UI matches that rather than inventing one) · ✅ A5-3 login/OTP support tooling 🔐 — shipped + merged 2026-07-10: `GET /api/admin/users/:userId/auth-state` (support-tier, read-only — B5 lockout snapshot + OTP request/verify timestamps + live-session count, never a code/hash) paired with `POST /api/admin/users/:userId/clear-otp-lockout` (the minimal unlock action, reuses the same `clearOtpAttempts` primitive verify-otp uses). Tier decision: admin-tier, NOT step-up-gated — modeled on the A5-2 revoke-sessions precedent (moves no value, self-limiting) rather than the support-tier delivery-unsticking actions, because it weakens the B5 brute-force defense for one account rather than re-driving paid-for work; unlike revoke-sessions it DOES require a `reason` + fires the Discord audit. `AuthStatePanel.tsx` on `/admin/users/:userId` — read for both staff roles, clear-lockout button admin-only · A5-5 operator-mediated DSR. (all 💰 unless tagged)
- [ ] **Test & E2E coverage (Q6):** ✅ Q6-2 money/auth workers (done 2026-07-07) · ✅ Q6-3 web money-write (done 2026-07-10 — admin-write envelope step-up + idempotency-key client coverage, #1628) · ✅ Q6-4 loop-native purchase-through-the-UI E2E (done 2026-07-10 — merged 2026-07-10: touches a one-line product fix in `PurchaseContainer.tsx` alongside the new e2e suite; see `money-auth-worklist.md` Q6-4) · ✅ Q6-5 admin/support UI E2E (done 2026-07-10 — self-merged 🟢, test-only: `tests/e2e-admin-support/` smokes the admin dashboard as a real staff user for both tiers, incl. the ADR 028 step-up gate triggering on redrive/refund and a full end-to-end revoke-sessions write; see `money-auth-worklist.md` Q6-5) · ✅ Q6-6 wallet-spend + interest-mint (done 2026-07-10 — real-postgres coverage; see `money-auth-worklist.md` Q6-6) · ✅ Q6-7 promote real-chain run off manual (done 2026-07-10 — self-merged 🟢, pure CI-config: `e2e-real.yml` gained a weekly `schedule:` trigger + graceful secrets-missing skip + Discord failure notify; script unchanged. **Still open (👤):** operator must provision `LOOP_E2E_REFRESH_TOKEN` + `STELLAR_TEST_SECRET_KEY` (+ `GH_SECRETS_PAT` + `DISCORD_WEBHOOK_MONITORING`) as repo secrets before the schedule actually executes rather than skipping; see `money-auth-worklist.md` Q6-7) · ✅ Q6-8 ratchet web floors (done 2026-07-10, #1630). 💰
- [x] **Q6-1** ctx-settlements direct counted coverage. 💰
- [x] **Fraud/abuse (B-3):** ✅ Phase 1 shipped + merged 2026-07-10: ADR 045 design + per-user order-create velocity limit (`fraud/velocity.ts`, bounded/indexed, fail-closed, count + per-currency value dimensions) + duplicate-account shared-funding-source detection (`fraud/duplicate-account-signals.ts`, flag-only, `fraud_signals` table + Discord page, never auto-block). Chargeback handling scoped as a documented T3/Plaid-card hook (no chargeback threat exists on Phase-1 crypto-only funding); device/IP dup-account capture and an admin UI for `fraud_signals` explicitly deferred (see ADR 045 Consequences). 💰 + design/ADR.

## §T1-BS · Hardening / blind-spots (before real growth)

- [ ] **B-1** load/stress/soak testing (absent) 🟢 **2026-07-09: harness half done** — k6 suite (`tools/load-test/`: browse + auth→order scenarios, SLO-derived thresholds) + `run-local.sh` + `.github/workflows/load-test.yml` (`workflow_dispatch`-only, not a required check) landed; dev-machine + mock-CTX baselines recorded in `docs/load-testing.md`. **Still open (👤):** the real breaking-point run against staging/a scratch Fly deploy — needs operator-provisioned infra, see `docs/load-testing.md` "What this does NOT cover". · [ ] **B-2** accessibility / WCAG 2.1 AA audit (EU exposure; a11y-contract tests started this session; 2026-07-09: mechanical floor landed — `eslint-plugin-jsx-a11y` lint gate + 5 `jest-axe` route smokes, ADR 042 — manual keyboard/screen-reader pass + contrast checking still open, see readiness-backlog B-2) 🟢 · [ ] **B-4** DR: PITR + offsite backup 👤+🟢 **2026-07-09: docs/procedure half done** — `docs/runbooks/disaster-recovery.md` rewritten with an operator checklist, corrected Fly-docs-cited backup-posture facts, 3-layer target posture (retention → PITR → offsite `pg_dump`), restore-drill procedure, and PROPOSED RPO/RTO tables (readiness-backlog B-4). **Still open (👤):** actually enabling PITR, provisioning the offsite bucket, building the `pg_dump` workflow, and running a timed restore drill. · [ ] **B-5** observability depth (Prometheus scraping tier/dashboards/alerts — endpoint exists) 🟢+👤 **2026-07-09: 🟢 code/config half done** — `docs/observability.md` + committed `docs/observability/{prometheus.yml,grafana-dashboard.json}` (scrape-config-as-code + a dashboard with one row per `docs/slo.md` section); `/metrics` gained `loop_catalog_stale`/`loop_catalog_loaded_timestamp_ms` (Freshness SLO), `loop_worker_stale`/`loop_worker_last_lead_tick_timestamp_ms` (S4-8 wedged-fleet signal), `loop_geo_db_stale`/`loop_geo_db_build_age_days`, and `loop_rate_limit_fleet_estimate`/`_source` gauges (readiness-backlog B-5). **Still open (👤+`[code]`):** standing up a real Prometheus + Grafana instance (or Grafana Cloud), OpenTelemetry tracing across order→payment-watcher→procurement→payout, and a paging tier (PagerDuty/Twilio) for P0 signals — see `docs/observability.md` "Operator actions". · [ ] **B-6** i18n (English-only scaffold, hardcoded copy, no RTL — large) 🟢 **2026-07-10: framework + first three extraction tranches done** — i18next/react-i18next chosen + wired (ADR 043), route-driven locale, SSR + static-export safe; tranche 1 (home/auth/onboarding/footer/404) + tranche 2 (settings/orders/gift-card detail) + tranche 3 (MobileHome/brand.$slug/remaining onboarding screens/map/Phase-2-gated cashback pages) extracted, catalogs English-only at `apps/web/app/i18n/locales/en/`, see `docs/i18n.md`. Supersedes the old CF-22 scaffold. `MAX_SSR_KB` raised 3300→3400 for tranche-2 catalog growth (`scripts/check-bundle-budget.sh`); tranche 3 stayed within that ceiling (3340/3400 KB). **Still open:** the purchase flow + `Phase2Gate.tsx`'s shared "Coming soon" panel + actual translations, both gated on the 🧭 language-set decision; RTL still unverified against a real language.
...
- [x] **MapLibre GL JS** swap for Leaflet (WebGL, better mobile marker perf). 🟢 **Done 2026-07-10 (#1638, ADR 046):** WebGL renderer against the existing CARTO raster tiles (no new tile infra); server-side `/api/clusters` clustering unchanged; independent review caught + fixed a production-CSP worker/tile block (`worker-src`/`connect-src`, empirically verified under the real CSP) before merge. Self-hosted vector tiles (Protomaps vs. paid) deferred to a 👤 tile-hosting decision.
- [x] **Referral program.** 🟢/💰 **Design done 2026-07-10 (#1641, ADR 047):** codes + `referral_redemptions` (UNIQUE on referee = one reward ever), reward as a conserved credit emission (INV-1-safe), first-`fulfilled`-order trigger, abuse model reuses B-3 (shared-funding-source + velocity). Active build gated behind the cashback flip (`LOOP_PHASE_1_ONLY`) + the 🧭 reward-economics decision — deferred.
- [x] **Analytics** (privacy-respecting, no PII) + **Core Web Vitals / API-latency perf monitoring.** 🟢+👤 **Done 2026-07-10 (#1640, ADR 048):** first-party cookieless CWV via `web-vitals` → rate-limited `POST /api/public/rum` → `/metrics` histograms, env-gated dark-by-default, no third-party vendor / consent banner. **Still open (👤):** operator flips `VITE_ANALYTICS_ENABLED` on + a Grafana panel.
- [x] **Thin-currency promotion process** — ADR 035 leaves ~20 catalogue-only currencies below the ≥15-merchant bar; define a review cadence (one-line `countries.ts` add each). 🟢+🧭 **Done 2026-07-10 (#1637):** quarterly cadence + threshold rationale + a reused `/api/merchants/all?fields=lite` measurement recipe + the promotion checklist — `docs/thin-currency-promotion.md`. Each actual promotion stays a 🧭 operator go/no-go.
- [x] **Extended-market order path tail** — currency CHECKs + `convertMinorUnits` for AED/INR/SAR/AUD/MXN (comprehensive-audit Part IV Phase 3; partly wired via CF-19). 🟢 **Done 2026-07-10 (#1639):** investigation found the order path was ALREADY fully wired by CF-19 (stale checkbox); the PR corrected stale ADR-035/architecture/error-code docs (Frankfurter FX, not CTX rates, is the real gate; AUD/INR/MXN live today, AED/SAR still vendor-gated as USD-pegged) + added the missing per-currency minor-unit + accept-path tests.
- [x] Scripts-pile disposition + audit quality tail (comprehensive-audit Part IV phases 9–10). 🟢 **Done 2026-07-10 (#1636):** inventoried `scripts/` + `tools/`, archived 2 consumed one-shots to `scripts/archive/` (0 removed, all live tooling kept), added `scripts/README.md` + `tools/README.md` indexes so the pile can't silently regrow.
