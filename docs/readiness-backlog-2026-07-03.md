...
- [x] **Status:** ✅ RESOLVED 2026-07-10 (operator decision — recorded with A5-4). Decision: **NO auto-refund.** The existing redemption-backfill exhaustion Discord page (`notifyRedemptionBackfillExhausted`, `orders/redemption-backfill.ts`, fires after 10 attempts/~17h) + a manual operator decision IS the policy — auto-refunding on exhaustion is unsafe because a `fulfilled`-but-no-code order MAY have been genuinely paid to CTX (CTX could still deliver, or the customer could already have a usable code), which is exactly the reconcile-before-refunding hazard this item flagged. When the operator decides a refund is warranted, it now flows through **THIS A5-4 fulfilled-order-refund-with-attestation path** (`POST /api/admin/orders/:orderId/refund` — the order is `fulfilled`, so the operator supplies the code-unused attestation affirming no usable code was delivered). The runbook (`docs/runbooks/redemption-backfill-exhausted.md`) already walks the CTX-paid-vs-unpaid diagnosis and the manual-refund branch; the linkage to the A5-4 endpoint is documented there. Nothing to auto-implement — the safe outcome is the manual, attested, audited refund, not a blind auto-refund.
...
- [ ] **Status:** 🟡 Code shipped 2026-07-10 (merged 2026-07-10); operator channel-provisioning still open
      ...
      **2026-07-10:** design + Phase-1 code shipped — `docs/adr/044-payout-throughput.md` picks **channel accounts** (the standard Stellar scale pattern) over operator/issuer account sharding or batching: N pre-funded channel accounts own the tx source (sequence number + fee) while the Payment op's `source` override keeps the operator/issuer as the real funder, so N sequence streams submit concurrently with zero risk of racing each other's `tx_bad_seq` — no change to the money-holding topology the "account sharding" alternative would have required. Implemented: `payments/channel-accounts.ts` (resolves `LOOP_STELLAR_PAYOUT_CHANNEL_SECRETS`, boot-validated in `env.ts` — format + no dupes + no collision with operator/issuer accounts), `submitPayout` gained an additive `channelSecret` (unset = byte-identical pre-ADR-044 behaviour), and `payout-worker.ts` shards a claimed batch across configured channels and runs shards concurrently via `Promise.all` (each shard still strictly serial against its own channel). The A8 fleet-wide leader lock is UNCHANGED (deliberately — see the ADR's "Why keep the fleet-wide leader lock" section); parallelism is bounded to N **within** one leader's tick, which the ADR argues is sufficient for the volumes named above. **Shipped with N=0 (unset) as the default — production behaviour is unchanged until an operator provisions channel accounts.** INV-9 argument + full test list in the ADR. **Still open (👤):** operator provisions + funds N channel accounts, sets `LOOP_STELLAR_PAYOUT_CHANNEL_SECRETS`, confirms real throughput scaling — leave this checkbox unticked until that lands (load-tested per B-1, per the original "Done when").
...
- [x] **Status:** ✅ Shipped 2026-07-09 (merged 2026-07-10 (#1609)).
...
- [x] **Status:** ✅ Shipped 2026-07-10 (merged 2026-07-10).
...
- [x] **Status:** ✅ Shipped 2026-07-10, merged 2026-07-10: `GET /api/admin/users/:userId/auth-state` (support-tier, read-only) returns the B5 verify-otp lockout snapshot (`otp_attempt_counters`: `locked`/`lockedUntil`/`failedAttempts`), the last `request-otp`/successful-`verify-otp` timestamps (aggregates over `otps.createdAt`/`otps.consumedAt` — never a code or a code hash), and a live-session count (`refresh_tokens` rows unrevoked + unexpired). Paired with `POST /api/admin/users/:userId/clear-otp-lockout`, the minimal unlock action for the common support case (legit user fat-fingered the code, is locked out, needs back in): deletes the user's `otp_attempt_counters` row via the SAME `clearOtpAttempts` primitive a successful `verify-otp` already uses (`auth/otp-attempt-counter.ts`) — no bespoke unlock path. **Tier decision (the crux, reasoned in the PR):** admin-tier, NOT step-up-gated — modeled on the A5-2 `revoke-sessions` precedent rather than the ADR 037 support-tier delivery-unsticking actions (wallet reprovision / watcher-skip reopen / redemption re-fetch), because this action WEAKENS the B5 brute-force defense for one account rather than re-driving already-paid-for work; kept at admin-tier so a compromised or socially-engineered support session can't unilaterally reopen an arbitrary account's guess budget. Not step-up-gated because, like revoke-sessions, it moves no value and is self-limiting (any further wrong guess re-arms the same lockout from a clean window) — step-up friction on a fast unlock is counterproductive. Unlike revoke-sessions it DOES require a `reason` (2..500 chars) and fires the Discord admin-audit fanout (ADR-017-lite: `Idempotency-Key` + reason + `{result, audit}`, no step-up header) because clearing a brute-force defense is a more security-relevant event than a session sign-out. Idempotent: clearing an already-clear (or never-locked) counter is a no-op success (`wasLocked: false`). Never leaks an OTP code, a code hash, or a refresh-token hash on either endpoint. **Per-target velocity cap (review-P1 fix):** the per-IP route limit (20/min) does NOT bound the "clear → guess → clear" B5-defeat loop (it only needs ~1 clear/60s, and a bearer can spread clears across IPs onto one victim), so the clear ALSO enforces a rolling-24h cap of `CLEAR_LOCKOUT_MAX_PER_TARGET_PER_DAY` (5) APPLIED clears per target userId, counted from the existing `admin_idempotency_keys` audit rows (the path encodes the userId; a row exists only for a committed clear; replays don't inflate it — no new table). Reaching the cap returns `OTP_LOCKOUT_CLEAR_RATE_EXCEEDED` (429); the count FAILS CLOSED to `OTP_LOCKOUT_CLEAR_RATE_CHECK_UNAVAILABLE` (503) if the query errors, so a transient DB fault can't hand out a free uncounted clear. This collapses the admin-assisted guess ceiling on one account from the pre-B5 ~14,400/day back to ~960 + (5×10) ≈ ~1,010/day, restoring essentially all of B5's value even under a compromised admin bearer. Web: new "Auth / login state" panel (`AuthStatePanel.tsx`) on `/admin/users/:userId` — read section renders for both staff roles, the "Clear OTP lockout" button (via `ReasonDialog`) only for admin. Mechanical gates updated: `staff-route-gating.test.ts` (both mounts pinned at their tiers, `clear-otp-lockout` added to the step-up-exempt list with its reasoning) and `rate-limit-route-inventory.test.ts` (generic walk, no manual entry needed). Out of scope (deliberately, per the task fence): OTP resend, impersonation, password reset — OTP delivery-failure visibility (`sent`/`delivery_status` on `otps`) is a separate, not-yet-started follow-up if it turns out to matter in practice.
...
- [x] **Status:** ✅ Shipped 2026-07-10 (merged 2026-07-10): `POST /api/admin/orders/:orderId/refund` — a NEW order-bound endpoint (distinct from the user-bound `POST /api/admin/users/:userId/refunds`, which stays as-is), admin-tier + step-up (new ADR-028 `order-refund` scope), ADR-017 envelope (Idempotency-Key + required reason + audit). **The operator DECIDED the policy** (built to exactly this): `paid`/`procuring`/`failed` orders refund directly; a **`fulfilled`** order (customer already has the code) IS refundable but ONLY behind a required **code-unused attestation** (`{ codeUnused: true, attestationNote }`) — the operator affirms the delivered code is unused/unusable, the compensating control standing in for the CTX redemption-verification Loop doesn't have. `pending_payment`/`expired` refused (400 `ORDER_NOT_REFUNDABLE`); fulfilled-without-attestation refused (400 `ORDER_REFUND_ATTESTATION_REQUIRED`); already-refunded refused (409 `ORDER_ALREADY_REFUNDED`). **No new money path** — dispatches to the SAME primitives auto-refund uses per `payment_method`: xlm/usdc → `applyOrderAutoRefund` → A6 `refundDeposit` on-chain-to-sender; credit → `applyAdminRefund` mirror-credit (real admin actor); **loop_asset → fails closed** (409 `ORDER_REFUND_UNSUPPORTED_PAYMENT_METHOD`, matching the R3-2 posture — escalate). `paid`/`procuring` orders are fenced to `failed` BEFORE the refund so the worker/A5-1 redrive can't later pay CTX for a refunded order — via a **state-pinned CAS** (`markOrderFailedFromState`, new in `transitions.ts`, `WHERE state=<exact validated state>`, NOT the broad `markOrderFailed`; money-review P2-1 fix: the broad predicate could fail a row a worker just claimed `paid→procuring` while that worker goes on to pay CTX → double-loss; the pinned fence returns null → 409 instead, refunding nothing); a `procuring` order carries two extra gates that make its refund exactly as safe as the recovery sweep's auto-refund (the only other writer that refunds a procuring order) — it must be **stale** (`procured_at` older than the sweep's 15-min terminal cutoff, closing the live-worker TOCTOU the A5-1 redrive refused procuring orders over — a fresh procuring order is refused 400 `ORDER_NOT_REFUNDABLE`) AND Loop must NOT have already paid CTX (`loopPaidCtx`, the same disambiguation the sweep uses → 409 `ORDER_REFUND_CTX_ALREADY_PAID` otherwise). **INV-8-idempotent** — rides the migration-0013 partial-unique index + the deposit-refund claim cross-check, no new uniqueness logic; a double-refund → exactly one refund. Web: admin-only `RefundOrderPanel` on `/admin/orders/:orderId` — reason dialog + step-up; for a fulfilled order a prominent double-spend warning + a REQUIRED "code is unused/unusable" checkbox + attestation note (submit disabled until ticked). The double-spend risk is a KNOWN, operator-accepted risk (attestation + audit mitigate it) — recorded in `docs/threat-model.md`. Tests: `apps/backend/src/admin/__tests__/order-refund.test.ts` (per-method dispatch, fulfilled-requires-attestation 400/200, already-refunded 409, ctx-paid-procuring refused, loop*asset fail-closed, fence-race, submit-fail 502, idempotent replay) + `apps/web/app/components/features/admin/__tests__/RefundOrderPanel.test.tsx` + `staff-route-gating.test.ts` / `rate-limit-route-inventory.test.ts` updates.
      **Why:** `POST /api/admin/users/:userId/refunds` (admin + step-up, order-bound, dupe-guarded) had **no UI** — curl only — and **never checked order state** and there was **no gift-card clawback** (`credits/refunds.ts` deliberately doesn't touch the order state machine): the user keeps the card \_and* the money; Loop eats it.
...
- [x] **Status:** ✅ Fixed 2026-07-10 — re-verified against current `main` first: the backend read (`GET /api/admin/stuck-orders` / `/stuck-payouts`, `admin-dashboard.ts`) already rides the ADR 037 support-tier blanket as of PR #1430 (2026-06-12) — it carries NO explicit `requireStaff('admin')` gate, so it was already a support-visible "blanket rider" by the time this item was written (the original `admin-dashboard.ts:39` citation predates ADR 037 landing). The real, verified gap was purely on the **web** side: `admin.stuck-orders.tsx` wrapped its content in `<RequireAdmin>` (admin-tier) and `admin._index.tsx` hid `StuckOrdersCard` behind `isAdminRole` + gave the nav card `minRole: 'admin'` — so a signed-in support agent hit a client-side deny banner even though the API would have served them. Fixed: the page now gates on `<RequireStaff minimum="support">`, the dashboard card renders unconditionally (matching `StuckPayoutsCard`, which was already unconditional — the two cards had drifted out of sync), and the nav-card `minRole` is `'support'`. Money-moving actions reached from the drill pages (order re-drive, payout retry) are untouched — still admin + step-up on their own buttons/endpoints. Added: an explicit support-200/non-staff-404 pin for both endpoints in `staff-route-gating.test.ts` (previously only implied by the generic "blanket riders" bucket-count assertion), and a regression test pinning that the stuck-orders triage default (5 min) fires strictly before `sweepStuckProcurement`'s terminal 15-min cutoff (`PROCUREMENT_TIMEOUT_MS`, now exported from a new dependency-free `orders/procurement-constants.ts` so the pin doesn't drag in the full procurement graph) — so ops always sees a row here before the sweep silently resolves it. PR: review-first, merged 2026-07-10.
...
- [x] **Status:** ✅ Shipped 2026-07-10, merged 2026-07-10 (#1622): `GET /api/admin/users/:userId/audit` — merges FIVE existing, already-bounded per-user reads (admin_idempotency_keys rows naming this userId, credit_transactions, orders, pending_payouts, refresh_tokens revocations) plus a current-state OTP-lock snapshot (otp_attempt_counters) into one newest-first timeline, rather than literally adding subject/actor filters to `audit-tail.ts` (that endpoint's underlying table has no per-user index and is TTL-capped at 24h — filtering it in place wouldn't have answered "what happened to this user" across money/orders/sessions anyway; the merged-timeline shape does). `?limit=` bounds EACH source independently (default 8, clamped [1, 20] — a maximally-populated default response is 41 events, under the CF-10 `BULK_LIST_ROW_THRESHOLD` of 50); pagination uses PER-SOURCE COMPOUND `(timestamp, id)` keyset cursors (the response's `nextCursors` `{at,id}` objects → the request's `beforeAdminActions`/`beforeLedger`/`beforeOrders`/`beforePayouts`/`beforeSessions` params, encoded `"<iso>|<id>"`), NOT one shared cursor and NOT a bare timestamp — two independent-review P1s fixed here: (1) a single global cursor would drop rows from denser sources under uneven density; (2) a bare-timestamp cursor would drop rows when > limit rows share one timestamp (deterministically triggered by `revokeAllRefreshTokensForUser` stamping one `revokedAt` on every session, and interest-mint's transaction-stable `now()` per credit row). The `id` tiebreaker (uuid / `refresh_tokens.jti` / `admin_idempotency_keys.key`) makes each source's keyset a total order. Support-tier (ADR 037 §3 lists "audit" among the shared read views, matching the A5-8 ledger browser's gating). Two documented, inherited limitations (not introduced by this PR): admin actions only cover the trailing 24h (admin_idempotency_keys is an hourly-swept idempotency cache, not a durable log — same window as the existing fleet-wide audit-tail), and the OTP-lock entry is a snapshot of current state, not a history (no append-only OTP-attempt log exists to draw from — that's A5-3). Web: new "Audit timeline" section on `/admin/users/:userId` (`UserAuditTimeline.tsx`), each row drill-linking to its order/payout detail page. Also closed, in the same PR: a tracked P2 from A5-8 — `/api/admin/ledger`'s default page size (50) equalled the CF-10 `BULK_LIST_ROW_THRESHOLD` (50), so every default page load tripped the bulk-read Discord tripwire; fixed via a per-path threshold override (`PER_PATH_BULK_ROW_THRESHOLD['/api/admin/ledger'] = 51`) in `read-audit.ts`.
...
- [x] **Drift-correction: ✅ RESOLVED 2026-07-10 (operator decision — recorded with A5-4).** Decision: **NO new drift-write primitive.** Ledger/off-chain drift is corrected via the EXISTING per-user admin credit-adjust (`POST /api/admin/users/:userId/credit-adjustments`, step-up + reason + audit), which is already guarded by the conservation trigger (`assert_emission_conservation`, INV-3) and the daily admin-write cap (INV-5) — the very fences a bespoke "drift-write" primitive would have to re-implement. It is DRIVEN by the read-only investigation tools that already shipped: the A5-8 fleet-wide ledger browser + the R3-1 float-reconciliation surface locate the drift; the operator then writes a corrective adjustment with a `reason` citing the reconciliation. A new unguarded write primitive is exactly the "silently demote an invariant to convention" failure mode `docs/invariants.md` warns against, so we deliberately don't build one. The correction procedure is documented in `docs/runbooks/ledger-drift.md` (§Resolution → "Money flow bug") and `docs/runbooks/operator-float-drift.md`. ⚠️ Money-review any real correction: it must stay within the conservation trigger (never mint unbacked value).
- [ ] **Bulk actions: ☐ Not started.** Reopen/retry/refund are one-at-a-time; add bulk selection. (Deferred — the drift-correction half above is the money-review-heavy part; bulk selection is UX ergonomics over the existing single-item, already-guarded writes.)
...
- [x] **Status:** ✅ Done 2026-07-10 — merged 2026-07-10 — added `tests/e2e-loop-purchase/purchase-flow.test.ts` +
...
- [x] **Status:** ✅ Done 2026-07-10 — merged 2026-07-10. Q6-4
...
- [x] **Status:** ✅ Phase 1 shipped 2026-07-10 (design + build, ADR 045, merged 2026-07-10). **Design:** `docs/adr/045-fraud-abuse-controls.md`. **Built:** (1) per-user order-create velocity limit — bounded/indexed query in `fraud/velocity.ts`, gates `POST /api/orders/loop` BEFORE order creation, count + per-currency value dimensions, both independently disable-able, fails CLOSED on a query error (429 `ORDER_VELOCITY_EXCEEDED` / 503 `ORDER_VELOCITY_CHECK_UNAVAILABLE`); (2) duplicate-account detection — shared-funding-source signal in `fraud/duplicate-account-signals.ts`, flag-only (`fraud_signals` table, migration 0059, Discord page on first occurrence), called from `payments/watcher.ts` after a paid transition commits, never auto-blocks. **Deferred (documented in ADR 045, not silently dropped):** device/signup-IP capture for a second dup-account signal (needs new PII capture + its own privacy review); a real chargeback/dispute state machine (no chargeback threat exists on Phase-1 crypto-only funding — blocked on the T3 Plaid/card funding-rail decision); an admin UI for `fraud_signals` (queryable directly + Discord page is the Phase-1 surface per the "don't over-build" guidance); auto-blocking on either control. Pairs with L1-1. ⚠️ Money + abuse-model review (money-reviewer subagent pass + independent human review before merge).
