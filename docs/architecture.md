...
merchantStore: {
  ...
  loadedAt:        number                // unix ms (last FULL sweep) — drives /health staleness check
}                                        // full-loaded at boot, ws-maintained, swept hourly (hardcoded)
...
**Merchant catalog loading is boot-load + websocket-maintained, operator-scoped.** At startup the backend pulls the full catalog at CTX's maximum page size (`GET /merchants?perPage=100000` — `httprpc.MaxPerPage`), paging further only if the response spans more than one page. The request carries Loop's operator API creds, so the catalog is the set CTX serves Loop — merchants linked to Loop's company, each with its per-operator effective `status` (authoritative over the global `enabled` flag) and any per-link user-discount override (which beats the merchant default in `savingsPercentage`). What CTX links/enables is what Loop sells; there is no Loop-side override. It then subscribes to CTX's `/ws` **merchant topic** (`merchants/ws-maintainer.ts`, authenticated with the operator `X-Api-Key`/`X-Api-Secret` on the upgrade request) and applies `system.merchant.{created,updated,status_changed,deleted}` — plus `system.merchantlink.{created,updated,status_changed}`, which CTX delivers on the same topic resolved to the same merchant-shaped payload — straight into the store, so both merchant edits AND link changes (linking Loop, per-link status/discount updates) are visible on Loop's API within a second, not at the next sweep. Event payloads run through the exact same Zod schema, `mapUpstreamMerchant`, and denylist filter as the sweep; upserts replace **in place** so catalog order (and `/api/merchants` pagination) stays stable. Ws-driven changes persist to the Postgres snapshot on a 30s debounce. Resilience layering: reconnect with exponential backoff (1s→60s), a full resync on every reconnect (missed events are unrecoverable — the ws has no replay), and an hourly full sweep (hardcoded — deliberately not a config knob) still runs as the belt-and-braces reconciler. Without the operator API creds the maintainer stays off and the sweep alone provides freshness (the pre-ws behaviour). `/health` exposes the connection state as `merchantWs: disabled | connecting | connected`.

**Merchant images cache-bust on `updatedAt`.** CTX's `updated` timestamp is carried onto `Merchant.updatedAt` and passed by the web app as the image proxy's `v` query param (`getImageProxyUrl(..., { version })`). `v` is part of the proxy's LRU cache key (never forwarded upstream) and changes the browser-cached URL — so an image swapped in place at the same URL propagates as soon as the catalog carries the new timestamp, instead of waiting out the two 7-day immutable caches.
...
GET  /api/admin/holds                                    [support — NS-08: list all active account holds (AML review)]
GET  /api/admin/users/:userId/holds                      [support — NS-08: list a user's account holds]
POST /api/admin/users/:userId/holds                      [admin + step-up — NS-08: place an AML hold (debits_only/full), ADR-017]
POST /api/admin/holds/:holdId/release                    [admin + step-up — NS-08: release an account hold, ADR-017]
