...
**Merchant catalog loading is boot-load + websocket-maintained.** At startup the backend pulls the full catalog at CTX's maximum page size (`GET /merchants?perPage=100000` — `httprpc.MaxPerPage`), paging further only if the response spans more than one page. It then subscribes to CTX's `/ws` **merchant topic** (`merchants/ws-maintainer.ts`, authenticated with the operator `X-Api-Key`/`X-Api-Secret` on the upgrade request) and applies `system.merchant.{created,updated,status_changed,deleted}` events straight into the store — an upstream merchant edit is visible on Loop's API within a second, not at the next sweep. Event payloads run through the exact same Zod schema, `mapUpstreamMerchant`, and denylist filter as the sweep; upserts replace **in place** so catalog order (and `/api/merchants` pagination) stays stable. Ws-driven changes persist to the Postgres snapshot on a 30s debounce. Resilience layering: reconnect with exponential backoff (1s→60s), a full resync on every reconnect (missed events are unrecoverable — the ws has no replay), and an hourly full sweep (hardcoded — deliberately not a config knob) still runs as the belt-and-braces reconciler. Without the operator API creds the maintainer stays off and the sweep alone provides freshness (the pre-ws behaviour). `/health` exposes the connection state as `merchantWs: disabled | connecting | connected`.
