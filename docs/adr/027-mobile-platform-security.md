...
The Loop mobile binary is a Capacitor v8 shell (apps/mobile) that loads a static React Router build. It carries the cashback flow end-to-end: bearer-token auth, on-device Stellar key generation, gift-card code reveal, withdrawal initiation. The audit (A2-1204) flagged that none of the platform-native binary-integrity controls are wired:
...
| **Jailbreak / root detection**  | App-lock on cold start (PR #288), Keychain-backed refresh tokens (ADR-006), bearer tokens kept in memory only (Zustand). A rooted device can still bypass app-lock by reading process memory, but the on-device Stellar secret is ephemeral and only signed against from the live in-memory key. | Rooted-device user can extract refresh token + replay until refresh-token rotation (CTX-side, every refresh call).                                                                            | Phase-2 fraud-rate signals OR App Store rejection.                                               |
