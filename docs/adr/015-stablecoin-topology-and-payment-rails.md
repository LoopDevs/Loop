...
Status: Accepted (2026-04-21); amended 2026-05-05 with **pending** partial supersessions by ADR 030 and ADR 031 — both of which are still **Proposed** (see their "Gate for Accepted" sections), so nothing in this ADR is superseded in the running system yet. **Live today (this ADR remains authoritative):** §composite-key `user_credits`, §FX-pin at order creation, §watcher LOOP-asset allowlist, §asset matrix, §home currency, and the implemented rollout checklist; the live asset codes are still USDLOOP / GBPLOOP / EURLOOP (`packages/shared/src/loop-asset.ts`, `apps/backend/src/credits/payout-asset.ts`). **Superseded once ADR 030 is Accepted + shipped:** §"User wallet linking" and rollout checklist items #362 / #366 (external-link model → Privy embedded wallet). **Superseded once ADR 031 is Accepted + shipped:** §"Loop issues three branded Stellar assets" partially (USDLOOP/EURLOOP retired in favour of Loop-curated DeFindex vault shares LOOPUSD/LOOPEUR; GBPLOOP retained with nightly on-chain 3% APY mints) and §"Defindex deposit automation — currently manual ops top-up" (replaced by Loop-curated vaults).
