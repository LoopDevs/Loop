...
| `src/users-me.ts`                      | A2-1505 `/api/users/me*` response shapes (13 types; `UserMeView.staffRole` added by ADR 037)                                                                                                |
...
| `src/staff-roles.ts`                   | ADR 037 staff-role shapes — `StaffRole`, `AdminStaffMember`, `AdminStaffListResponse`, grant/revoke result payloads                                                                         |
| `src/admin-watcher-skips.ts`           | ADR 037 `/api/admin/watcher-skips*` shapes — `WatcherSkipStatus`/`WatcherSkipReason` unions (mirror the table CHECKs), row/detail/reopen                                                    |
| `src/admin-user-wallet.ts`             | ADR 037 admin wallet view — `AdminUserWalletView` + reprovision result (reuses `users-wallet.ts` provisioning/balance shapes)                                                               |
| `src/admin-lookup.ts`                  | ADR 037 `GET /api/admin/lookup` — order-id / memo / address → user reverse lookup                                                                                                           |
| `src/admin-order-redemption.ts`        | ADR 037 `POST /api/admin/orders/:id/refetch-redemption` result shape                                                                                                                        |
